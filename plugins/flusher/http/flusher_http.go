...
func (f *FlusherHTTP) Init(context pipeline.Context) error {
	...
	if f.RemoteURL == "" {
		...
		logger.Error(f.context.GetRuntimeContext(), util.PLUGIN_INIT_ALARM, "http flusher init fail, error", err)
		return err
	}

	if f.Concurrency < 1 {
		err := errors.New("concurrency must be greater than zero")
		logger.Error(f.context.GetRuntimeContext(), util.PLUGIN_INIT_ALARM, "http flusher check concurrency fail, error", err)
		return err
	}

	var err error
	if err = f.initEncoder(); err != nil {
		logger.Error(f.context.GetRuntimeContext(), util.PLUGIN_INIT_ALARM, "http flusher init encoder fail, error", err)
		return err
	}

	if err = f.initConverter(); err != nil {
		logger.Error(f.context.GetRuntimeContext(), util.PLUGIN_INIT_ALARM, "http flusher init converter fail, error", err)
		return err
	}
	...
	if f.FlushInterceptor != nil {
		...
		if err != nil {
			logger.Error(f.context.GetRuntimeContext(), util.PLUGIN_INIT_ALARM, "http flusher init filter fail, error", err)
			return err
		}
...
func (f *FlusherHTTP) initHTTPClient() error {
	...
	if f.Authenticator != nil {
		...
		if err != nil {
			logger.Error(f.context.GetRuntimeContext(), util.PLUGIN_INIT_ALARM, "http flusher init authenticator fail, error", err)
			return err
		}
...
func (f *FlusherHTTP) initRequestInterceptors(transport http.RoundTripper) (http.RoundTripper, error) {
	for i := len(f.RequestInterceptors) - 1; i >= 0; i-- {
		...
		if err != nil {
			logger.Error(f.context.GetRuntimeContext(), util.PLUGIN_INIT_ALARM, "http flusher init request interceptor fail, error", err)
			return nil, err
		}
...
func (f *FlusherHTTP) addTask(log interface{}) {
	...
	if f.DropEventWhenQueueFull {
		...
		default:
			f.counter.Done()
			logger.Warningf(f.context.GetRuntimeContext(), util.PLUGIN_RUNTIME_ALARM, "http flusher dropped a group event since the queue is full")
		}
	} else {
		f.queue <- log
	}
...
func (f *FlusherHTTP) runFlushTask() {
	...
	for data := range f.queue {
		...
		if err != nil {
			logger.Errorf(f.context.GetRuntimeContext(), util.PLUGIN_RUNTIME_ALARM,
...
func (f *FlusherHTTP) encodeAndFlush(event any) error {
	...
	for _, shard := range data {
		if err = f.flushWithRetry(shard, nil); err != nil {
			logger.Error(f.context.GetRuntimeContext(), util.PLUGIN_RUNTIME_ALARM,
...
func (f *FlusherHTTP) convertAndFlush(data interface{}) error {
	...
	if err != nil {
		logger.Error(f.context.GetRuntimeContext(), util.PLUGIN_RUNTIME_ALARM, "http flusher converter log fail, error", err)
		return err
	}
	...
	case [][]byte:
		for idx, data := range rows {
			...
			if err != nil {
				logger.Error(f.context.GetRuntimeContext(), util.PLUGIN_RUNTIME_ALARM, "http flusher failed flush data after retry, data dropped, error", err)
			}
		}
		return nil
	case []byte:
		...
		if err != nil {
			logger.Error(f.context.GetRuntimeContext(), util.PLUGIN_RUNTIME_ALARM, "http flusher failed flush data after retry, error", err)
		}
		return err
	default:
		err = fmt.Errorf("not supported logs type [%T]", logs)
		logger.Error(f.context.GetRuntimeContext(), util.PLUGIN_RUNTIME_ALARM, "http flusher failed flush data, error", err)
		return err
	}
}
...
func (f *FlusherHTTP) flush(data []byte, varValues map[string]string) (ok, retryable bool, err error) {
	...
	if err != nil {
		logger.Error(f.context.GetRuntimeContext(), util.PLUGIN_RUNTIME_ALARM, "create reader error", err)
		return false, false, err
	}

	req, err := http.NewRequest(http.MethodPost, f.RemoteURL, reader)
	if err != nil {
		logger.Error(f.context.GetRuntimeContext(), util.PLUGIN_RUNTIME_ALARM, "http flusher create request fail, error", err)
		return false, false, err
	}
	...
	if len(f.Query) > 0 {
		...
		for k, v := range f.Query {
			...
			if ferr != nil {
				logger.Error(f.context.GetRuntimeContext(), util.PLUGIN_RUNTIME_ALARM, "http flusher format query fail, error", ferr)
			} else {
				v = *fv
			}
			values.Add(k, v)
	...
	for k, v := range f.Headers {
		...
		fv, ferr := fmtstr.FormatTopic(varValues, v)
		if ferr != nil {
			logger.Error(f.context.GetRuntimeContext(), util.PLUGIN_RUNTIME_ALARM, "http flusher format header fail, error", ferr)
		} else {
			v = *fv
		}
		req.Header.Add(k, v)
	...
	if err != nil {
		...
		logger.Error(f.context.GetRuntimeContext(), util.PLUGIN_RUNTIME_ALARM, "http flusher send request fail, error", err)
		return false, retry, err
	}
	body, err := io.ReadAll(response.Body)
	if err != nil {
		logger.Error(f.context.GetRuntimeContext(), util.PLUGIN_RUNTIME_ALARM, "http flusher read response fail, error", err)
		return false, false, err
	}
	err = response.Body.Close()
	if err != nil {
		logger.Warning(f.context.GetRuntimeContext(), util.PLUGIN_RUNTIME_ALARM, "http flusher close response body fail, error", err)
		return false, false, err
	}
	...
	case 5:
		logger.Error(f.context.GetRuntimeContext(), util.PLUGIN_RUNTIME_ALARM, "http flusher write data returned error, url", req.URL.String(), "status", response.Status, "body", string(body))
...
func (f *FlusherHTTP) buildVarKeys() {
	...
	for _, define := range defines {
		for _, v := range define {
			...
			if err != nil {
				logger.Warning(f.context.GetRuntimeContext(), util.PLUGIN_INIT_ALARM, "http flusher init varKeys fail, err", err)
			}
