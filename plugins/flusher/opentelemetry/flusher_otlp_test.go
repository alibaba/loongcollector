...
func Test_Flusher_Init(t *testing.T) {
	...
	convey.Convey("When config is invalid", t, func() {
		convey.Convey("When FlusherOTLP is not initialized", func() {
			f := &FlusherOTLP{Version: v1, Logs: &helper.GrpcClientConfig{Endpoint: addr}}
			err := f.Stop()
			convey.So(err, convey.ShouldBeNil)
		})
	})
	convey.Convey("When init grpc service", t, func() {
		_, server := newTestGrpcService(t, addr, time.Nanosecond)
		defer func() {
			server.Stop()
		}()
	...
	convey.Convey("When init grpc service", t, func() {
		...
		convey.Convey("When FlusherOTLP init", func() {
			...
			convey.Convey("When FlusherOTLP flush", func() {
				...
				err := f.Export(PipelineGroupEventsSlice, helper.NewNoopPipelineContext())
				convey.So(err, convey.ShouldBeNil)
				r := <-service.ch
