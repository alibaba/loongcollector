...
func (f *FlusherOTLP) Init(ctx pipeline.Context) error {
	...
	if err != nil {
		logger.Warning(f.context.GetRuntimeContext(), selfmonitor.FlusherInitAlarm, "init otlp converter fail, error", err)
		return err
	}
	f.converter = convert
	...
	if f.Logs != nil {
		...
		if err != nil {
			logger.Warning(f.context.GetRuntimeContext(), selfmonitor.FlusherInitAlarm, "init otlp logs gRPC conn fail, error", err)
		} else {
	...
	if f.Metrics != nil {
		...
		if err != nil {
			logger.Warning(f.context.GetRuntimeContext(), selfmonitor.FlusherInitAlarm, "init otlp metrics gRPC conn fail, error", err)
		} else {
	...
	if f.Traces != nil {
		...
		if err != nil {
			logger.Warning(f.context.GetRuntimeContext(), selfmonitor.FlusherInitAlarm, "init otlp traces gRPC conn fail, error", err)
		} else {
	...
	if f.logClient == nil && f.metricClient == nil && f.traceClient == nil {
		logger.Warning(f.context.GetRuntimeContext(), selfmonitor.FlusherInitAlarm, "init otlp flusher fail, error", "invalid gRPC configs")
...
func (f *FlusherOTLP) IsReady(projectName string, logstoreName string, logstoreKey int64) bool {
	...
	if !ready {
		logger.Warning(f.context.GetRuntimeContext(), selfmonitor.FlusherReadyAlarm, "otlp flusher is not ready, all gRPC conn is nil")
	}
	return ready
}
...
func (f *FlusherOTLP) Stop() error {
	...
	if f.logClient != nil && f.logClient.grpcConn != nil {
		...
		if err != nil {
			logger.Warning(f.context.GetRuntimeContext(), selfmonitor.FlusherStopAlarm, "stop otlp logs flusher fail, error", err)
		}
	}
	...
	if f.metricClient != nil && f.metricClient.grpcConn != nil {
		...
		if err != nil {
			logger.Warning(f.context.GetRuntimeContext(), selfmonitor.FlusherStopAlarm, "stop otlp metrics flusher fail, error", err)
		}
	}
	...
	if f.traceClient != nil && f.traceClient.grpcConn != nil {
		...
		if err != nil {
			logger.Warning(f.context.GetRuntimeContext(), selfmonitor.FlusherStopAlarm, "stop otlp traces flusher fail, error", err)
		}
	}
...
func (f *FlusherOTLP) flushRequests(log plogotlp.ExportRequest, metric pmetricotlp.ExportRequest, trace ptraceotlp.ExportRequest) error {
	...
	if f.logClient != nil {
		...
		go func() {
			...
			if err != nil {
				logger.Warning(f.context.GetRuntimeContext(), selfmonitor.FlusherFlushAlarm, "send data to otlp server fail, error", err)
			}
			errChan <- err
		}()
	}
	...
	if f.metricClient != nil {
		...
		go func() {
			...
			if err != nil {
				logger.Warning(f.context.GetRuntimeContext(), selfmonitor.FlusherFlushAlarm, "send metric data to otlp server fail, error", err)
			}
			errChan <- err
		}()
	}
	...
	if f.traceClient != nil {
		...
		go func() {
			...
			if err != nil {
				logger.Warning(f.context.GetRuntimeContext(), selfmonitor.FlusherFlushAlarm, "send trace data to otlp server fail, error", err)
			}
			errChan <- err
		}()
