...
func (f *FlusherLoki) Init(context pipeline.Context) error {
	...
	if err := f.Validate(); err != nil {
		logger.Error(f.context.GetRuntimeContext(), util.PLUGIN_INIT_ALARM, "init loki flusher error", err)
		return err
	}
	...
	if err != nil {
		logger.Error(f.context.GetRuntimeContext(), util.PLUGIN_INIT_ALARM, "init loki flusher converter error", err)
		return err
	}
	f.converter = convert
...
func (f *FlusherLoki) Flush(projectName string, logstoreName string, configName string, logGroupList []*protocol.LogGroup) error {
	for _, logGroup := range logGroupList {
		...
		if err != nil {
			logger.Error(f.context.GetRuntimeContext(), util.PLUGIN_RUNTIME_ALARM, "flush loki convert log fail, error", err)
			continue
		}
