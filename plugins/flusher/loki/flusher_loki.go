...
func (f *FlusherLoki) Init(context pipeline.Context) error {
	...
	if err := f.Validate(); err != nil {
		logger.Warning(f.context.GetRuntimeContext(), selfmonitor.FlusherInitAlarm, "init loki flusher error", err)
		return err
	}
	...
	if err != nil {
		logger.Warning(f.context.GetRuntimeContext(), selfmonitor.FlusherInitAlarm, "init loki flusher converter error", err)
		return err
	}
	f.converter = convert
