...
func (f *FlusherElasticSearch) Init(context pipeline.Context) error {
	...
	if err := f.Validate(); err != nil {
		logger.Warning(f.context.GetRuntimeContext(), selfmonitor.FlusherInitAlarm, "init elasticsearch flusher fail, error", err)
		return err
	}
	...
	if err != nil {
		logger.Warning(f.context.GetRuntimeContext(), selfmonitor.FlusherInitAlarm, "init elasticsearch flusher converter fail, error", err)
		return err
	}
	f.converter = convert
	...
	if err != nil {
		logger.Warning(f.context.GetRuntimeContext(), selfmonitor.FlusherInitAlarm, "init elasticsearch flusher index fail, error", err)
		return err
	}
	...
	if err = f.Authentication.ConfigureAuthenticationAndHTTP(f.HTTPConfig, &cfg); err != nil {
		...
		logger.Warning(f.context.GetRuntimeContext(), selfmonitor.FlusherInitAlarm, "init elasticsearch flusher error", err)
		return err
	}
