...
func (f *FlusherElasticSearch) Init(context pipeline.Context) error {
	...
	if err := f.Validate(); err != nil {
		logger.Error(f.context.GetRuntimeContext(), util.PLUGIN_INIT_ALARM, "init elasticsearch flusher fail, error", err)
		return err
	}
	...
	if err != nil {
		logger.Error(f.context.GetRuntimeContext(), util.PLUGIN_INIT_ALARM, "init elasticsearch flusher converter fail, error", err)
		return err
	}
	f.converter = convert
	...
	if err != nil {
		logger.Error(f.context.GetRuntimeContext(), util.PLUGIN_INIT_ALARM, "init elasticsearch flusher index fail, error", err)
		return err
	}
	...
	if err = f.Authentication.ConfigureAuthenticationAndHTTP(f.HTTPConfig, &cfg); err != nil {
		...
		logger.Error(f.context.GetRuntimeContext(), util.PLUGIN_INIT_ALARM, "init elasticsearch flusher error", err)
		return err
	}

	f.esClient, err = elasticsearch.NewClient(cfg)
	if err != nil {
		logger.Error(f.context.GetRuntimeContext(), util.PLUGIN_INIT_ALARM, "create elasticsearch client error", err)
		return err
	}
	return nil
}
...
func (f *FlusherElasticSearch) Flush(projectName string, logstoreName string, configName string, logGroupList []*protocol.LogGroup) error {
	...
	for _, logGroup := range logGroupList {
		...
		if err != nil {
			logger.Error(f.context.GetRuntimeContext(), util.PLUGIN_RUNTIME_ALARM, "flush elasticsearch convert log fail, error", err)
			return err
		}
		var buffer []string
		for index, log := range serializedLogs.([][]byte) {
			...
			if f.isDynamicIndex {
				...
				if err != nil {
					logger.Error(f.context.GetRuntimeContext(), util.PLUGIN_RUNTIME_ALARM, "flush elasticsearch format index fail, error", err)
					return err
				}
			}
		...
		if err != nil {
			logger.Error(f.context.GetRuntimeContext(), util.PLUGIN_RUNTIME_ALARM, "flush elasticsearch request fail, error", err)
			return err
		}
		defer res.Body.Close()

		if res.StatusCode >= 400 && res.StatusCode <= 499 {
			logger.Error(f.context.GetRuntimeContext(), util.PLUGIN_RUNTIME_ALARM, "flush elasticsearch request client error", res)
			return fmt.Errorf("err status returned: %v", res.Status())
		} else if res.StatusCode >= 500 && res.StatusCode <= 599 {
			logger.Error(f.context.GetRuntimeContext(), util.PLUGIN_RUNTIME_ALARM, "flush elasticsearch request server error", res)
			return fmt.Errorf("err status returned: %v", res.Status())
		}
