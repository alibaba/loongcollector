...
func (f *FlusherClickHouse) Init(context pipeline.Context) error {
	...
	if err := f.Validate(); err != nil {
		logger.Error(f.context.GetRuntimeContext(), util.FlusherInitAlarm, "init clickhouse flusher fail, error", err)
		return err
	}
	...
	if err != nil {
		logger.Error(f.context.GetRuntimeContext(), util.FlusherInitAlarm, "init clickhouse flusher converter fail, error", err)
		return err
	}
	f.converter = convert
	conn, err := newConn(f)
	if err != nil {
		logger.Error(f.context.GetRuntimeContext(), util.FlusherInitAlarm, "init clickhouse flusher error", err)
		return err
	}
	f.conn = conn
...
func (f *FlusherClickHouse) BufferFlush(projectName string, logstoreName string, configName string, logGroupList []*protocol.LogGroup) error {
	...
	for _, logGroup := range logGroupList {
		...
		if err != nil {
			logger.Error(f.context.GetRuntimeContext(), util.FlusherFlushAlarm, "flush clickhouse convert log fail, error", err)
		}
		for _, log := range serializedLogs.([][]byte) {
			...
			if err != nil {
				logger.Error(f.context.GetRuntimeContext(), util.FlusherFlushAlarm, "flush clickhouse AsyncInsert fail, error", err)
			}
		}
...
func newConn(f *FlusherClickHouse) (driver.Conn, error) {
	...
	if err = conn.Ping(context.Background()); err != nil {
		if exception, ok := err.(*clickhouse.Exception); ok {
			logger.Error(f.context.GetRuntimeContext(), util.FlusherInitAlarm, "code", exception.Code, "msg", exception.Message, "trace", exception.StackTrace)
		} else {
			logger.Error(f.context.GetRuntimeContext(), util.FlusherInitAlarm, "error", err)
		}
		return nil, err
	}
	return conn, nil
...
func createNullBufferTable(f *FlusherClickHouse) error {
	...
	if err := f.conn.Exec(context.Background(), sqlNull); err != nil {
		logger.Error(f.context.GetRuntimeContext(), util.FlusherInitAlarm, "sql", sqlNull, "error", err)
		return err
	}
	...
	if err := f.conn.Exec(context.Background(), sqlBuffer); err != nil {
		logger.Error(f.context.GetRuntimeContext(), util.FlusherInitAlarm, "sql", sqlBuffer, "error", err)
		return err
	}
	return nil
}
