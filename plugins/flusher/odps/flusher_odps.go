...
import (
	...
	"github.com/alibaba/ilogtail/plugins/flusher/datahub"
...
func NewOdpsFlusher() *OdpsFlusher {
	return &OdpsFlusher{
		...
		TimeRange:       15,
		PartitionConfig: "",
		encryptor:       datahub.NewDefaultEncryptor(),
	}
}
...
func (o *OdpsFlusher) CreateTunnel() (*tunnel.Tunnel, error) {
	...
	if len(o.BearerToken) > 0 {
		...
		accessKeySecret, err := o.encryptor.DecodeKey(o.AccessKeySecret)
		if err != nil {
			logger.Errorf(o.context.GetRuntimeContext(), OdpsAlarm, "odps(%s/%s) decode access key secret failed, error:%v", o.ProjectName, o.TableName, err)
			return nil, err
		}

		if len(o.SecurityToken) > 0 {
			aliAccount = account.NewStsAccount(o.AccessKeyId, accessKeySecret, o.SecurityToken)
		} else {
			aliAccount = account.NewAliyunAccount(o.AccessKeyId, accessKeySecret)
		}
	}
