...
func (f *Flusher) Init(ctx pipeline.Context) error {
	...
	if f.EnableTLS {
		...
		if err != nil {
			logger.Errorf(f.ctx.GetRuntimeContext(), util.PLUGIN_INIT_ALARM, "error in creating TLS config,: %v", err)
			return err
		}
	...
	if err != nil {
		logger.Errorf(f.ctx.GetRuntimeContext(), util.PLUGIN_INIT_ALARM, "error in dialing with gRPC server %s, would try again later", f.Address)
	} else {
		f.conn = c
...
func (f *Flusher) IsReady(projectName string, logstoreName string, logstoreKey int64) bool {
	...
	if err != nil {
		logger.Errorf(f.ctx.GetRuntimeContext(), util.PLUGIN_RUNTIME_ALARM, "error in dialing with gRPC server %s, would try again later", f.Address)
		return false
	}
	f.conn = c
	f.client = protocol.NewLogReportServiceClient(c)
...
func (f *Flusher) Flush(projectName string, logstoreName string, configName string, logGroupList []*protocol.LogGroup) error {
	...
	if err != nil {
		logger.Error(f.ctx.GetRuntimeContext(), util.PLUGIN_RUNTIME_ALARM, "err", err)
		return err
	}
