...
func (f *Flusher) Init(ctx pipeline.Context) error {
	...
	if f.EnableTLS {
		...
		if err != nil {
			logger.Errorf(f.ctx.GetRuntimeContext(), util.FlusherFlushAlarm, "grpc flush error in creating TLS config,: %v", err)
			return err
		}
	...
	if err != nil {
		logger.Errorf(f.ctx.GetRuntimeContext(), util.FlusherFlushAlarm, "grpc flush error in dialing with gRPC server %s, would try again later", f.Address)
	} else {
		f.conn = c
...
func (f *Flusher) Flush(projectName string, logstoreName string, configName string, logGroupList []*protocol.LogGroup) error {
	...
	if err != nil {
		logger.Error(f.ctx.GetRuntimeContext(), util.FlusherFlushAlarm, "grpc flush err", err)
		return err
	}
