...
func (k *FlusherKafka) Init(context pipeline.Context) error {
	...
	if k.Brokers == nil || len(k.Brokers) == 0 {
		...
		logger.Error(k.context.GetRuntimeContext(), util.PLUGIN_INIT_ALARM, "init kafka flusher fail, error", err)
		return err
	}
	...
	if err != nil {
		logger.Error(k.context.GetRuntimeContext(), util.PLUGIN_INIT_ALARM, "init kafka flusher converter fail, error", err)
		return err
	}
	k.converter = convert
	...
	go func(p sarama.AsyncProducer, SIGTERM chan bool) {
		...
		for {
			...
			case err := <-errors:
				if err != nil {
					logger.Error(k.context.GetRuntimeContext(), util.PLUGIN_RUNTIME_ALARM, "flush kafka write data fail, error", err)
				}
			case <-success:
				// Do Nothing
			case <-SIGTERM:
...
func (k *FlusherKafka) NormalFlush(projectName string, logstoreName string, configName string, logGroupList []*protocol.LogGroup) error {
	...
	for _, logGroup := range logGroupList {
		...
		if err != nil {
			logger.Error(k.context.GetRuntimeContext(), util.PLUGIN_RUNTIME_ALARM, "flush kafka convert log fail, error", err)
		}
		for index, log := range logs.([][]byte) {
			if len(k.topicKeys) > 0 {
				...
				if err != nil {
					logger.Error(k.context.GetRuntimeContext(), util.PLUGIN_RUNTIME_ALARM, "flush kafka format topic fail, error", err)
				} else {
...
func newSaramaConfig(config *FlusherKafka) (*sarama.Config, error) {
	...
	if err := k.Validate(); err != nil {
		logger.Error(config.context.GetRuntimeContext(), util.PLUGIN_INIT_ALARM, "Invalid kafka configuration, error", err)
		return nil, err
	}
	return k, nil
}
