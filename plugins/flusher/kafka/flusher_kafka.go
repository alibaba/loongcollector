...
func (k *FlusherKafka) Init(context pipeline.Context) error {
	...
	if len(k.SASLUsername) == 0 {
		logger.Warning(k.context.GetRuntimeContext(), util.PLUGIN_INIT_ALARM, "SASL information is not set, access Kafka server without authentication")
	} else {
	...
	case "random":
		partitioner = sarama.NewRandomPartitioner
	default:
		logger.Error(k.context.GetRuntimeContext(), util.PLUGIN_INIT_ALARM, "invalid PartitionerType, use RandomPartitioner instead, type", k.PartitionerType)
	}
