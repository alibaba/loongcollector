...
func (k *FlusherKafka) Init(context pipeline.Context) error {
	...
	if len(k.SASLUsername) == 0 {
		logger.Warning(k.context.GetRuntimeContext(), selfmonitor.FlusherInitAlarm, "SASL information is not set, access Kafka server without authentication")
	} else {
	...
	case "random":
		partitioner = sarama.NewRandomPartitioner
	default:
		logger.Warning(k.context.GetRuntimeContext(), selfmonitor.InvalidKafkaPartitioner, "invalid PartitionerType, use RandomPartitioner instead, type", k.PartitionerType)
	}
