...
func (f *FlusherPulsar) Init(context pipeline.Context) error {
	...
	if err := f.Validate(); err != nil {
		logger.Error(f.context.GetRuntimeContext(), util.PLUGIN_INIT_ALARM, "init pulsar flusher fail, error", err)
		return err
	}
	...
	if err != nil {
		logger.Error(f.context.GetRuntimeContext(), util.PLUGIN_INIT_ALARM, "init pulsar flusher converter fail, error", err)
		return err
	}
	f.converter = convert
	...
	if err != nil {
		logger.Error(f.context.GetRuntimeContext(), util.PLUGIN_INIT_ALARM, "init pulsar flusher producer options fail, error", err)
		return err
	}
...
func (f *FlusherPulsar) Flush(projectName string, logstoreName string, configName string, logGroupList []*protocol.LogGroup) error {
	...
	for _, logGroup := range logGroupList {
		...
		if err != nil {
			logger.Error(f.context.GetRuntimeContext(), util.PLUGIN_RUNTIME_ALARM, "flush pulsar convert log fail, error", err)
		}
		for index, log := range logs.([][]byte) {
			...
			if len(f.topicKeys) > 0 {
				...
				if err != nil {
					logger.Error(f.context.GetRuntimeContext(), util.PLUGIN_RUNTIME_ALARM, "flush pulsar format topic fail, error", err)
				} else {
			...
			if err != nil {
				logger.Error(f.context.GetRuntimeContext(), util.PLUGIN_RUNTIME_ALARM, "load pulsar producer fail,topic", topic, "err", err)
				return err
			}
			...
			producer.SendAsync(f.context.GetRuntimeContext(), message, func(msgId pulsar.MessageID, prodMsg *pulsar.ProducerMessage, err error) {
				if err != nil {
					logger.Error(f.context.GetRuntimeContext(), util.PLUGIN_RUNTIME_ALARM, "send message to pulsar fail,error", err)
				} else {
...
func (f *FlusherPulsar) Stop() error {
	...
	if err != nil {
		logger.Error(f.context.GetRuntimeContext(), util.PLUGIN_STOP_ALARM, "stop pulsar flusher fail, error", err)
	}
...
func (f *FlusherPulsar) hashPartitionKey(valueMap map[string]string, defaultKey string) string {
	...
	if len(notMatchKeys) > 0 {
		logger.Warning(f.context.GetRuntimeContext(), util.PLUGIN_RUNTIME_ALARM, "Some fields in PartitionKeys cannot be matched in the log content, keys", notMatchKeys)
	}
	if len(hashData) == 0 {
