...
func (pi *ProducerImpl) freshShardIds(force bool) error {
	...
	if pi.shardInfo != nil {
		...
		pi.shardInfo = &ShardInfo{
			shardIds:      shardIds,
			index:         int(time.Now().UnixNano()), // 首次刷新需要指定一个随机起点
	...
	if len(pi.shardInfo.shardIds) > 0 {
		...
		// 如果没有active的shard，那么下次获取shard还会刷新shard列表
...
func (pi *ProducerImpl) DoSend(records []datahub.IRecord, isRecursive bool) (string, *datahub.PutRecordsByShardResult, error) {
	...
	if _, ok := err.(*datahub.ShardSealedError); ok {
		...
		if err != nil {
			...
			logger.Infof(pi.context.GetRuntimeContext(), "Fresh datahub (%s/%s) shard success", pi.projectName, pi.topicName)
...
func (pi *ProducerImpl) Send(records []datahub.IRecord) error {
	...
	for retry := 0; retry < maxRetryCount; retry++ {
		...
		if err == nil {
			logger.Infof(pi.context.GetRuntimeContext(), "Flush datahub(%s/%s/%s) success, rid:%s, records:%d, rawSize:%d, reqSize:%d, cost:%v",
