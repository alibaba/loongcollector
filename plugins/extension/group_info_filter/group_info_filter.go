...
func (e *ExtensionGroupInfoFilter) Init(context pipeline.Context) error {
	...
	for k, v := range e.Tags {
		...
		if err != nil {
			logger.Error(context.GetRuntimeContext(), util.ExtensionAlarm, "ext_groupinfo_filter failed, regex compile tags err, pattern", v, "error", err)
			return err
		}
		v.Reg = reg
		e.tagsPattern[k] = v
	...
	for k, v := range e.Metas {
		reg, err := regexp.Compile(v.Pattern)
		if err != nil {
			logger.Error(context.GetRuntimeContext(), util.ExtensionAlarm, "ext_groupinfo_filter failed, regex compile metas err, pattern", v, "error", err)
			return err
		}
		v.Reg = reg
		e.metasPattern[k] = v
