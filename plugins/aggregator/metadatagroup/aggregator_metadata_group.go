...
func (g *metadataGroup) collectByLengthChecker(group *models.PipelineGroupEvents, ctx pipeline.PipelineContext) {
	for {
		...
		if availableSize < 0 {
			logger.Warning(g.context.GetRuntimeContext(), selfmonitor.RuntimeAlarm, "availableSize is negative")
...
func (g *metadataGroup) collectByLengthAndBytesChecker(group *models.PipelineGroupEvents, ctx pipeline.PipelineContext) {
	for {
		...
		if availableLenSize < 0 || availableBytesSize < 0 {
			logger.Warning(g.context.GetRuntimeContext(), selfmonitor.RuntimeAlarm, "availableSize or availableLength is negative")
			_ = g.GetResultWithoutLock(ctx)
			continue
		}
		...
		if len(g.events) == 0 && num == 0 {
			...
			if !g.dropOversizeEvent {
				...
				logger.Warningf(g.context.GetRuntimeContext(), selfmonitor.AggregateOversizeAlarm, "event[%s] size [%d] is over the limit size %d, the event would be dropped",
