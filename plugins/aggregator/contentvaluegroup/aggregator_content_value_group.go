...
func (g *AggregatorContentValueGroup) getOrCreateGroupAggs(log *protocol.Log) (*groupAggregator, error) {
	...
	if _, err := agg.Init(g.context, groupQueue); err != nil {
		logger.Error(g.context.GetRuntimeContext(), util.AggregatorAddAlarm, "aggregator group fail to create agg for group", groupKVs)
		return nil, err
	}
	agg.InitInner(
		g.EnablePackID,
...
func (g *AggregatorContentValueGroup) getGroupKVs(log *protocol.Log) map[string]string {
	...
	for _, key := range g.GroupKeys {
		...
		if !found && g.ErrIfKeyNotFound {
			logger.Warning(g.context.GetRuntimeContext(), util.AggregatorAddAlarm, "aggregator group fail to find key in log content,key", key)
		}
		group[key] = val
	}
	return group
