...
func shouldCreatePath(p string) bool {
	...
	logger.Warningf(telegrafManager.GetContext(), util.PLUGIN_RUNTIME_ALARM,
		...
		if files, err := os.ReadDir(tm.telegrafConfPath); err == nil {
			for _, f := range files {
				...
				if err = os.Remove(filePath); err == nil {
					...
					logger.Warningf(telegrafManager.GetContext(), util.PLUGIN_RUNTIME_ALARM, "deleted outdated agent config file err, path: %v, err: %v",
						filePath, err)
				}
			}
		} else {
