...
func (r *InputGpuMetric) Start(collector pipeline.Collector) error {
	...
	if err != nil {
		logger.Error(r.context.GetRuntimeContext(), util.InputInitAlarm, "Couldn't initialize nvml, error", err)
		return err
	}
	...
	for {
		...
		case <-timer.C:
			...
			if err != nil {
				logger.Error(r.context.GetRuntimeContext(), util.InputCollectAlarm, "GPU collect metric error", err)
				return nil
			}
...
func (r *InputGpuMetric) CollectGpuMetric() error {
	...
	if err != nil {
		logger.Error(r.context.GetRuntimeContext(), util.InputCollectAlarm, "GPU DeviceCount error", err)
		return err
	}
	...
	for index := uint(0); index < numDevices; index++ {
		...
		if err != nil {
			logger.Error(r.context.GetRuntimeContext(), util.InputCollectAlarm, "GPU DeviceHandleByIndex", index, "error", err)
			return err
		}
