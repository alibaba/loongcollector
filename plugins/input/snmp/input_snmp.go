	...
	for index, GsAgent := range s.gs {
		...
		go func() {
			err := func() error {
				...
				if err != nil {
					logger.Errorf(context.Background(), util.InputStartAlarm, "snmp connection error, %v", err)
					return err
				}
				...
				for i, variable := range result.Variables {
					...
					if translatedType == "Null" || translatedType == "" {
						logger.Warning(context.Background(), util.InputStartAlarm, "result of %v is probably empty", s.fieldContents[i].Name)
						if s.Version == 1 {
							logger.Warning(context.Background(), util.InputStartAlarm, "snmp v1 will not return any `GET` result while one of them is empty, please check `Oids`, `Fields`, `Tables` or change `Version` into `2`")
						}
					}
			...
			if err != nil {
				logger.Errorf(context.Background(), util.InputStartAlarm, fmt.Sprintf("snmp connection error, %v", err))
			}
		}()
	}
	return nil
