...
func (m *Mysql) Init(context pipeline.Context) (int, error) {
	...
	if len(m.StateMent) == 0 && len(m.StateMentPath) != 0 {
		...
		if err != nil && len(data) > 0 {
			...
			logger.Warning(m.context.GetRuntimeContext(), selfmonitor.MysqlInitAlarm, "init sql statement error", err)
		}
	}
...
func (m *Mysql) initMysql() error {
	...
	if err != nil {
		logger.Warning(m.context.GetRuntimeContext(), selfmonitor.MysqlInitAlarm, "MySQL Error registering TLS config", err)
	}
		...
		if err == nil {
			if len(m.StateMent) > 0 {
				...
				if err == nil {
					...
					logger.Warning(m.context.GetRuntimeContext(), selfmonitor.MysqlInitAlarm, "init db statement error, err", err)
					break
				}
			}
		}
...
func (m *Mysql) dsnConfig() (string, error) {
	...
	if len(m.Location) > 0 {
		...
		if err != nil {
			...
			logger.Warning(m.context.GetRuntimeContext(), selfmonitor.MysqlInitAlarm, "Set Mysql location error, loc", m.Location, "error", err)
		}
	}
...
func (m *Mysql) Start(collector pipeline.Collector) error {
	...
	if m.CheckPoint {
		...
		if exist && len(val) > 0 {
			...
			switch {
			case err != nil:
				logger.Warning(m.context.GetRuntimeContext(), selfmonitor.MysqlCheckpoingAlarm, "init checkpoint error, key", m.CheckPointColumn, "value", string(val), "error", err)
			case cp.CheckPointColumn == m.CheckPointColumn && m.CheckPointColumnType == cp.CheckPointColumnType:
				m.checkpointValue = cp.Value
			default:
				logger.Warning(m.context.GetRuntimeContext(), selfmonitor.MysqlCheckpoingAlarm, "not matched checkpoint, may be config update, last column",
	...
	for {
		...
		case <-timer.C:
			...
			if err != nil {
				logger.Warning(m.context.GetRuntimeContext(), selfmonitor.MysqlQueryAlarm, "sql query error", err)
			}
			...
			if endTime.Sub(startTime) > time.Duration(m.IntervalMs)*time.Millisecond/2 {
				logger.Warning(m.context.GetRuntimeContext(), selfmonitor.MysqlTimeoutAlarm, "sql collect cost very long time, start", startTime, "end", endTime, "intervalMs", m.IntervalMs)
...
func (m *Mysql) SaveCheckPoint(collector pipeline.Collector) {
	...
	if err != nil {
		logger.Warning(m.context.GetRuntimeContext(), selfmonitor.MysqlCheckpointAlarm, "save checkpoint marshal error, checkpoint", cp, "error", err)
		return
	}
	...
	if err != nil {
		logger.Warning(m.context.GetRuntimeContext(), selfmonitor.MysqlCheckpointAlarm, "save checkpoint dump error, checkpoint", cp, "error", err)
	}
}
...
func (m *Mysql) ParseRows(rows *sql.Rows, collector pipeline.Collector) int {
	...
	if m.columnsKeyBuffer == nil {
		...
		if err != nil {
			logger.Warning(m.context.GetRuntimeContext(), selfmonitor.MysqlParseAlarm, "no columns info, use default columns info, error", err)
		}
		...
		if m.CheckPoint && len(m.CheckPointColumn) != 0 && !foundCheckpointColumn {
			logger.Warning(m.context.GetRuntimeContext(), selfmonitor.MysqlParseAlarm, "no checkpoint column", m.CheckPointColumn)
		}
	...
	for rows.Next() {
		...
		if err != nil {
			logger.Warning(m.context.GetRuntimeContext(), selfmonitor.MysqlParseAlarm, "scan error, row", rowCount, "error", err)
			return rowCount
		}
