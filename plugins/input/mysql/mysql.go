...
func (m *Mysql) initMysql() error {
	...
	if err != nil {
		logger.Error(m.context.GetRuntimeContext(), util.PLUGIN_INIT_ALARM, "MySQL Error registering TLS config", err)
	}
...
func (m *Mysql) dsnConfig() (string, error) {
	...
	if len(m.Location) > 0 {
		...
		if err != nil {
			...
			logger.Error(m.context.GetRuntimeContext(), util.PLUGIN_INIT_ALARM, "Set Mysql location error, loc", m.Location, "error", err)
		}
	}
...
func (m *Mysql) InitCheckPointFromString(val string) {
	...
	// 	if err != nil {
	// 		logger.Warning(util.PLUGIN_INIT_ALARM, "init time checkpoint error, value", val, "error", err)
...
func (m *Mysql) Start(collector pipeline.Collector) error {
	...
	for {
		...
		case <-timer.C:
			...
			if err != nil {
				logger.Error(m.context.GetRuntimeContext(), util.PLUGIN_RUNTIME_ALARM, "sql query error", err)
			}
			...
			if endTime.Sub(startTime) > time.Duration(m.IntervalMs)*time.Millisecond/2 {
				logger.Warning(m.context.GetRuntimeContext(), util.PLUGIN_RUNTIME_ALARM, "sql collect cost very long time, start", startTime, "end", endTime, "intervalMs", m.IntervalMs)
...
func (m *Mysql) ParseRows(rows *sql.Rows, collector pipeline.Collector) int {
	...
	if m.columnsKeyBuffer == nil {
		...
		if err != nil {
			logger.Warning(m.context.GetRuntimeContext(), util.PLUGIN_RUNTIME_ALARM, "no columns info, use default columns info, error", err)
		}
		...
		if m.CheckPoint && len(m.CheckPointColumn) != 0 && !foundCheckpointColumn {
			logger.Warning(m.context.GetRuntimeContext(), util.PLUGIN_RUNTIME_ALARM, "no checkpoint column", m.CheckPointColumn)
		}
	...
	for rows.Next() {
		...
		if err != nil {
			logger.Warning(m.context.GetRuntimeContext(), util.PLUGIN_RUNTIME_ALARM, "scan error, row", rowCount, "error", err)
			return rowCount
		}
