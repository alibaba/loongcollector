...
func (n *Nginx) Collect(collector pipeline.Collector) error {
	...
	for _, u := range n.Urls {
		...
		if err != nil {
			logger.Error(n.context.GetRuntimeContext(), util.PLUGIN_RUNTIME_ALARM, "Unable to parse address", u, "error", err)
		}
		...
		go func(addr *url.URL) {
			...
			if err != nil {
				logger.Error(n.context.GetRuntimeContext(), util.PLUGIN_RUNTIME_ALARM, "url", addr.Host, "error", err)
			}
		}(addr)
	}
