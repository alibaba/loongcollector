...
func (p *ServiceLumber) Start(c pipeline.Collector) error {
	...
	for {
		...
		if p.SSLCert == "" && p.SSLKey == "" && p.SSLCA == "" && !p.InsecureSkipVerify {
			...
			if err != nil {
				logger.Error(p.context.GetRuntimeContext(), util.PLUGIN_RUNTIME_ALARM, "init tls error", err)
			}
			p.server, err = server.ListenAndServe(p.BindAddress,
				server.V1(p.V1),
				server.V2(p.V2),
		...
		if err != nil {
			logger.Error(p.context.GetRuntimeContext(), util.PLUGIN_RUNTIME_ALARM, "listen init error", err, "sleep 10 seconds and retry")
	...
	ForBlock:
		for {
			...
			case batch := <-recvChan:
				if batch == nil {
					err = p.server.Close()
					logger.Error(p.context.GetRuntimeContext(), util.PLUGIN_RUNTIME_ALARM, "lumber server error", "chan closed", "close server, err", err)
					break ForBlock
				}
