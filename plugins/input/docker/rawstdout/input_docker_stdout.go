			...
			for {
				...
				if err != nil {
					if err != io.EOF && err != io.ErrClosedPipe {
						logger.Warning(ss.context.GetRuntimeContext(), util.PLUGIN_START_ALARM, "stdoutSyner done, id", ss.info.IDPrefix(),
			...
			for {
				...
				if matchRst := ss.beginLineReg.FindStringIndex(checkLine); len(matchRst) >= 2 && matchRst[0] == 0 && matchRst[1] == len(checkLine) {
					...
					if len(values[2]) >= ss.maxLogSize {
						logger.Warning(ss.context.GetRuntimeContext(), util.PLUGIN_START_ALARM, "log line is too long, force flush out", len(values[2]), "log prefix", util.CutString(values[2], 4096))
						c.AddDataArray(tags, keys, values)
						values[2] = ""
					}
				}
	...
	for {
		...
		if len(ss.startCheckPoint) > 0 {
			...
			if cpTime, err = time.Parse(helper.DockerTimeFormat, ss.startCheckPoint); err != nil {
				logger.Warning(ss.context.GetRuntimeContext(), util.CHECKPOINT_ALARM, "docker stdout raw parse start time error", ss.startCheckPoint,
					"id", ss.info.IDPrefix(),
		...
		if err != nil {
			logger.Errorf(ss.context.GetRuntimeContext(), util.PLUGIN_START_ALARM, "open container log error=%v, id:%v\tname:%v\tcreated:%v\tstatus:%v",
		...
		if ss.info.ContainerInfo.Config.Tty {
			...
			if err != nil && err != context.Canceled {
				logger.Errorf(ss.context.GetRuntimeContext(), util.PLUGIN_START_ALARM, "read container log error=%v, id:%v\tname:%v\tcreated:%v\tstatus:%v",
					err.Error(), ss.info.IDPrefix(), ss.info.ContainerInfo.Name, ss.info.ContainerInfo.Created, ss.info.Status())
			}
		} else {
		...
		if closeErr := logReader.Close(); closeErr != nil {
			logger.Warningf(ss.context.GetRuntimeContext(), util.PLUGIN_START_ALARM, "close container log error=%v, id:%v\tname:%v\tcreated:%v\tstatus:%v",
		...
		default:
			...
			logger.Warning(ss.context.GetRuntimeContext(), util.PLUGIN_START_ALARM, "stdoutSyner stop, retry after 10 seconds, id", ss.info.IDPrefix(),
				"name", ss.info.ContainerInfo.Name, "created", ss.info.ContainerInfo.Created, "status", ss.info.Status(), "error", err)
...
func (sds *ServiceDockerStdout) Init(context pipeline.Context) (int, error) {
	...
	if err != nil {
		logger.Warning(sds.context.GetRuntimeContext(), util.PLUGIN_INIT_ALARM, "init include env regex error", err)
	}
	sds.ExcludeEnv, sds.ExcludeEnvRegex, err = helper.SplitRegexFromMap(sds.ExcludeEnv)
	if err != nil {
		logger.Warning(sds.context.GetRuntimeContext(), util.PLUGIN_INIT_ALARM, "init exclude env regex error", err)
	}
	...
	if err != nil {
		logger.Warning(sds.context.GetRuntimeContext(), util.PLUGIN_INIT_ALARM, "init include label regex error", err)
	}
	sds.ExcludeLabel, sds.ExcludeLabelRegex, err = helper.SplitRegexFromMap(sds.ExcludeLabel)
	if err != nil {
		logger.Warning(sds.context.GetRuntimeContext(), util.PLUGIN_INIT_ALARM, "init exclude label regex error", err)
	}
		...
		if syner, ok := sds.synerMap[id]; !ok || firstStart {
			...
			if len(sds.BeginLineRegex) > 0 {
				if reg, err = regexp.Compile(sds.BeginLineRegex); err != nil {
					logger.Warning(sds.context.GetRuntimeContext(), util.PLUGIN_RUNTIME_ALARM, "compile begin line regex error, regex", sds.BeginLineRegex, "error", err)
				}
			}
			syner = &stdoutSyner{
				info:                 info,
...
func (sds *ServiceDockerStdout) Start(c pipeline.Collector) error {
	...
	if sds.client, err = helper.CreateDockerClient(); err != nil {
		logger.Error(sds.context.GetRuntimeContext(), util.DOCKER_CENTER_ALARM, "create docker client error", err)
		return err
	}
