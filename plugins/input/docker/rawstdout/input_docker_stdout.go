...
func (ss *stdoutSyner) Start(c pipeline.Collector) {
	...
	for {
		...
		if len(ss.startCheckPoint) > 0 {
			var err error
			if cpTime, err = time.Parse(dockercenter.DockerTimeFormat, ss.startCheckPoint); err != nil {
		...
		if cpTime.IsZero() {
			...
			logger.Info(ss.context.GetRuntimeContext(), "docker stdout raw first read since", cpTime.Format(dockercenter.DockerTimeFormat),
				"id", ss.info.IDPrefix(),
				"name", ss.info.ContainerInfo.Name, "created", ss.info.ContainerInfo.Created, "status", ss.info.Status())
		}
		ss.lock.Unlock()
		options := types.ContainerLogsOptions{
			...
			Since:      cpTime.Format(dockercenter.DockerTimeFormat),
			Timestamps: true,
			Follow:     true,
		}
...
func (sds *ServiceDockerStdout) FlushAll(c pipeline.Collector, firstStart bool) error {
	var err error
	dockerInfos := dockercenter.GetContainerByAcceptedInfo(
...
func (sds *ServiceDockerStdout) Start(c pipeline.Collector) error {
	...
	var err error
	if sds.client, err = dockercenter.CreateDockerClient(); err != nil {
