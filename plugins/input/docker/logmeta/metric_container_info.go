...
func (idf *InputDockerFile) Init(context pipeline.Context) (int, error) {
	...
	if err != nil {
		logger.Warning(idf.context.GetRuntimeContext(), util.PLUGIN_INIT_ALARM, "init include env regex error", err)
	}
	idf.ExcludeEnv, idf.ExcludeEnvRegex, err = helper.SplitRegexFromMap(idf.ExcludeEnv)
	if err != nil {
		logger.Warning(idf.context.GetRuntimeContext(), util.PLUGIN_INIT_ALARM, "init exclude env regex error", err)
	}
	...
	if err != nil {
		logger.Warning(idf.context.GetRuntimeContext(), util.PLUGIN_INIT_ALARM, "init include label regex error", err)
	}
	idf.ExcludeLabel, idf.ExcludeLabelRegex, err = helper.SplitRegexFromMap(idf.ExcludeLabel)
	if err != nil {
		logger.Warning(idf.context.GetRuntimeContext(), util.PLUGIN_INIT_ALARM, "init exclude label regex error", err)
	}
...
func (idf *InputDockerFile) addMappingToLogtail(info *helper.DockerInfoDetail, containerInfo ContainerInfoCache, allCmd *DockerFileUpdateCmdAll) {
	...
	if err := logtail.ExecuteCMD(configName, PluginDockerUpdateFile, cmdBuf); err != nil {
		logger.Error(idf.context.GetRuntimeContext(), util.PLUGIN_RUNTIME_ALARM, "cmdType", PluginDockerUpdateFile, "cmd", cmdBuf, "error", err)
	}
}
...
func (idf *InputDockerFile) deleteMappingFromLogtail(id string) {
	...
	if err := logtail.ExecuteCMD(configName, PluginDockerDeleteFile, cmdBuf); err != nil {
		logger.Error(idf.context.GetRuntimeContext(), util.PLUGIN_RUNTIME_ALARM, "cmdType", PluginDockerDeleteFile, "cmd", cmdBuf, "error", err)
	}
}
...
func (idf *InputDockerFile) notifyStopToLogtail(id string) {
	...
	if err := logtail.ExecuteCMD(configName, PluginDockerStopFile, cmdBuf); err != nil {
		logger.Error(idf.context.GetRuntimeContext(), util.PLUGIN_RUNTIME_ALARM, "cmdType", PluginDockerStopFile, "cmd", cmdBuf, "error", err)
	}
}
...
func (idf *InputDockerFile) updateAll(allCmd *DockerFileUpdateCmdAll) {
	...
	if err := logtail.ExecuteCMD(configName, PluginDockerUpdateFileAll, cmdBuf); err != nil {
		logger.Error(idf.context.GetRuntimeContext(), util.PLUGIN_RUNTIME_ALARM, "cmdType", PluginDockerUpdateFileAll, "cmd", cmdBuf, "error", err)
	}
}
