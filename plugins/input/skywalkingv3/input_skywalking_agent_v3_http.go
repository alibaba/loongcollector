	...
	return s.registerHandler(func(request *http.Request) (interface{}, error) {
		...
		for dec.More() {
			...
			if e := dec.Decode(&result); e != nil {
				logger.Error(s.context.GetRuntimeContext(), util.InputCollectAlarm, "skywalking failed to decode error log", e)
				continue
			} else {
...
func (s *SkywalkingHTTPServerInput) writeResponse(writer http.ResponseWriter, data []byte) {
	if _, e := writer.Write(data); e != nil {
		logger.Error(s.context.GetRuntimeContext(), util.InputCollectAlarm, "skywalking failed to write response", e)
	}
}
