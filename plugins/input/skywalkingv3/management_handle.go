...
func (r *ResourcePropertiesCache) save(ctx pipeline.Context) {
	...
	if err != nil {
		logger.Warning(ctx.GetRuntimeContext(), selfmonitor.SkywalkingSaveCheckpointFail, "err", err.Error())
	}
}
...
func (r *ResourcePropertiesCache) load(ctx pipeline.Context) bool {
	...
	if ok {
		...
		if err != nil {
			logger.Warning(ctx.GetRuntimeContext(), selfmonitor.SkywalkingLoadCheckpointFail, "err", err.Error())
			return false
		}
	}
	return true
