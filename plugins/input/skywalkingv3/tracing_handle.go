...
func panicRecover() {
	if err := recover(); err != nil {
		...
		logger.Warning(context.Background(), selfmonitor.PluginRuntimeAlarm, "skywalking v3 runtime panic error", err, "stack", string(trace))
	}
}
...
func (h *TracingHandler) CollectInSync(ctx context.Context, req *skywalking.SegmentCollection) (*v3.Commands, error) {
	...
	for _, segment := range req.Segments {
		...
		if err != nil {
			logger.Warning(h.context.GetRuntimeContext(), selfmonitor.SkywalkingCollectTraceError, "error", err)
			continue
		}
	}
	return &v3.Commands{}, nil
...
func (h *TracingHandler) collectSegment(segment *skywalking.SegmentObject, mapping map[int32]string) error {
	for _, span := range segment.Spans {
		...
		if otTrace == nil {
			logger.Warning(h.context.GetRuntimeContext(), selfmonitor.SkywalkingResourceNotReady, "err", "resource properties not found, drop this segment")
			continue
		}
		log, err := otTrace.ToLog()
		if err != nil {
			logger.Warning(h.context.GetRuntimeContext(), selfmonitor.SkywalkingToOtTraceErr, "err", err)
			return err
		}
