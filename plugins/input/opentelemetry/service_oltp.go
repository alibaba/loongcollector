...
func (s *Server) start() error {
	if s.Protocals.GRPC != nil {
		...
		if err != nil {
			logger.Warningf(s.context.GetRuntimeContext(), util.InputStartAlarm, "inavlid grpc server config: %v, err: %v", s.Protocals.GRPC, err)
		}
...
func (s *Server) registerHTTPLogsComsumer(serveMux *http.ServeMux, decoder extensions.Decoder, maxBodySize int64, routing string) {
	serveMux.HandleFunc(routing, func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			logger.Warning(s.context.GetRuntimeContext(), util.InputCollectAlarm, "export req logs failed", err, "request", r.URL.String())
			httpserver.BadRequest(w)
			return
		}
		...
		if err != nil {
			logger.Warning(s.context.GetRuntimeContext(), util.InputCollectAlarm, "marshal resp failed", err, "request", r.URL.String())
			httpserver.BadRequest(w)
			return
		}
...
func (s *Server) registerHTTPMetricsComsumer(serveMux *http.ServeMux, decoder extensions.Decoder, maxBodySize int64, routing string) {
	serveMux.HandleFunc(routing, func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			logger.Warning(s.context.GetRuntimeContext(), util.InputCollectAlarm, "export req metrics failed", err, "request", r.URL.String())
			httpserver.BadRequest(w)
			return
		}
...
func (s *Server) registerHTTPTracesComsumer(serveMux *http.ServeMux, decoder extensions.Decoder, maxBodySize int64, routing string) {
	serveMux.HandleFunc(routing, func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			logger.Warning(s.context.GetRuntimeContext(), util.InputCollectAlarm, "export req traces failed", err, "request", r.URL.String())
			httpserver.BadRequest(w)
			return
		}
