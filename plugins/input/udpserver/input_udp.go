...
func (u *UDPServer) Init(context pipeline.Context) (int, error) {
	...
	if err != nil {
		logger.Error(u.context.GetRuntimeContext(), util.InputInitAlarm, "service_udp_server init failed, illegal udp listening addr", u.Address, "err", err)
		return 0, err
	}
	if host == "" {
		host = "0.0.0.0"
	...
	if err != nil {
		logger.Error(u.context.GetRuntimeContext(), util.InputInitAlarm, "service_udp_server init failed, unable resolve addr", u.Address, "err", err)
		return 0, err
	}

	port, err := strconv.Atoi(portStr)
	if err != nil || port < 0 || port > 65535 {
		logger.Error(u.context.GetRuntimeContext(), util.InputInitAlarm, "service_udp_server init failed, illegal port", portStr, "err", err)
	}
...
func (u *UDPServer) doStart(dispatchFunc func(logs []*protocol.Log)) error {
	...
	if err != nil {
		logger.Error(u.context.GetRuntimeContext(), util.InputStartAlarm, "service_udp_server start failed, start udp server err", err)
		return err
	}
	go func() {
		...
		for {
			...
			if err != nil {
				...
				logger.Error(u.context.GetRuntimeContext(), util.InputStartAlarm, "service_udp_server start failed, read record err", err)
				return
			}
			logs, err := u.decoder.Decode(buf[:n], nil, nil)
			if err != nil {
				logger.Error(u.context.GetRuntimeContext(), util.InputStartAlarm, "service_udp_server start failed, decode record err,some logs would be dropped", err)
			} else {
				dispatchFunc(logs)
			}
		}
