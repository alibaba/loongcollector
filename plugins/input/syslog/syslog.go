...
func (s *Syslog) Start(collector pipeline.Collector) error {
	...
	if s.isStream {
		...
		if err != nil {
			logger.Error(s.context.GetRuntimeContext(), util.ServiceSyslogInitAlarm, "net.Listen error", err,
		...
		if err != nil {
			logger.Error(s.context.GetRuntimeContext(), util.ServiceSyslogInitAlarm, "net.ListenPacket error", err,
				"Address", s.Address, "scheme", scheme, "host", host)
			return err
		}
		s.Closer = l
...
func (s *Syslog) Stop() error {
	...
	if s.Closer != nil {
		if err := s.Close(); err != nil {
			logger.Error(s.context.GetRuntimeContext(), util.ServiceSyslogCloseAlarm, "Syslog.Close error", err,
				"Address", s.Address)
		}
	}
	s.wg.Wait()
	...
	if s.isUnix {
		...
		if err != nil {
			logger.Error(s.context.GetRuntimeContext(), util.ServiceSyslogCloseAlarm, "getAddressParts error", err,
				"Address", s.Address)
		}
		err = os.Remove(host)
		if err != nil {
			logger.Error(s.context.GetRuntimeContext(), util.ServiceSyslogCloseAlarm, "os.Remove error", err,
				"Host", host)
		}
	}
	return nil
...
Loop:
	for {
		...
		if err != nil {
			...
			logger.Error(s.context.GetRuntimeContext(), util.ServiceSyslogStreamAlarm, "accept error", err)
		...
		if err := s.setKeepAlive(tcpConn); err != nil {
			logger.Error(s.context.GetRuntimeContext(), util.ServiceSyslogStreamAlarm, "setKeepAlive error", err)
		}
...
func (s *Syslog) handle(conn net.Conn, collector pipeline.Collector) {
	...
	for scanner.Scan() {
		...
		if err != nil {
			...
			if strings.HasSuffix(err.Error(), ": i/o timeout") {
				logger.Warning(s.context.GetRuntimeContext(), util.ServiceSyslogStreamAlarm, "connection i/o timeout")
				s.resetTimeout(conn)
				continue
			}

			logger.Error(s.context.GetRuntimeContext(), util.ServiceSyslogStreamAlarm, "scan error", err)
	...
	if err := scanner.Err(); err != nil {
		logger.Error(s.context.GetRuntimeContext(), util.ServiceSyslogStreamAlarm,
...
func (s *Syslog) listenPacket(collector pipeline.Collector) {
	...
	for {
		...
		if err != nil {
			...
			if strings.HasSuffix(err.Error(), ": i/o timeout") {
				logger.Warning(s.context.GetRuntimeContext(), util.ServiceSyslogPacketAlarm, "connection i/o timeout")
				continue
			}

			logger.Error(s.context.GetRuntimeContext(), util.ServiceSyslogPacketAlarm, "read from error", err)
			if s.sleepWithChan(backoff.Next()) {
				break
			}
			continue
...
func (s *Syslog) parse(b []byte, clientIP string, collector pipeline.Collector) {
	...
	for _, line := range lines {
		...
		if err != nil {
			logger.Warning(s.context.GetRuntimeContext(), util.ServiceSyslogParseAlarm,
