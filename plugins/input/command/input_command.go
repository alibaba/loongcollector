...
func (in *InputCommand) Init(context pipeline.Context) (int, error) {
	...
	if valid, err := in.Validate(); err != nil || !valid {
		logger.Error(in.context.GetRuntimeContext(), util.CATEGORY_CONFIG_ALARM, "init input_command error", err)
		return 0, err
	}
...
func (in *InputCommand) Validate() (bool, error) {
	...
	if in.TimeoutMilliSeconds > in.IntervalMs {
		...
		logger.Warning(in.context.GetRuntimeContext(), util.CATEGORY_CONFIG_ALARM, "init input_command warning", "TimeoutMilliSeconds > IntervalMs", "set TimeoutMilliSeconds = IntervalMs")
	}
...
func (in *InputCommand) Collect(collector pipeline.Collector) error {
	...
	if err != nil {
		if in.IgnoreError {
			...
			logger.Error(in.context.GetRuntimeContext(), util.PLUGIN_RUNTIME_ALARM, "input_command Collect error", err)
		}
		return err
	}
