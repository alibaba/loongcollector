...
func (m *Jmx) Init(context pipeline.Context) (int, error) {
	...
	if m.DiscoveryMode {
		var err error
		m.IncludeEnv, m.includeEnvRegex, err = dockercenter.SplitRegexFromMap(m.IncludeEnv)
		if err != nil {
			logger.Warning(m.context.GetRuntimeContext(), "INVALID_REGEX_ALARM", "init include env regex error", err)
		}
		m.ExcludeEnv, m.excludeEnvRegex, err = dockercenter.SplitRegexFromMap(m.ExcludeEnv)
		if err != nil {
			logger.Warning(m.context.GetRuntimeContext(), "INVALID_REGEX_ALARM", "init exclude env regex error", err)
		}
		m.IncludeContainerLabel, m.includeContainerLabelRegex, err = dockercenter.SplitRegexFromMap(m.IncludeContainerLabel)
		if err != nil {
			logger.Warning(m.context.GetRuntimeContext(), "INVALID_REGEX_ALARM", "init include label regex error", err)
		}
		m.ExcludeContainerLabel, m.excludeContainerLabelRegex, err = dockercenter.SplitRegexFromMap(m.ExcludeContainerLabel)
		if err != nil {
			logger.Warning(m.context.GetRuntimeContext(), "INVALID_REGEX_ALARM", "init exclude label regex error", err)
		}
		m.k8sFilter, err = dockercenter.CreateK8SFilter(m.K8sNamespaceRegex, m.K8sPodRegex, m.K8sContainerRegex, m.IncludeK8sLabel, m.ExcludeK8sLabel)
		if err != nil {
...
func (m *Jmx) UpdateContainerCfg() {
	...
	containers := dockercenter.GetContainerByAcceptedInfo(m.IncludeContainerLabel, m.ExcludeContainerLabel,
