...
func panicRecover() {
	if err := recover(); err != nil {
		...
		logger.Error(context.Background(), util.PluginRuntimeAlarm, "skywalking v2 runtime panic error", err, "stack", string(trace))
	}
}
...
func (t *TraceSegmentReportHandle) collectSegment(server v2.TraceSegmentReportService_CollectServer, upstream *agent.UpstreamSegment) error {
	...
	for _, span := range segment.Spans {
		...
		if otTrace == nil {
			...
			if err != nil {
				logger.Error(t.context.GetRuntimeContext(), util.InputCollectAlarm, "skywalking to ot trace err", err)
				return err
			}
