...
func (j *JVMMetricServiceHandle) Collect(ctx context.Context, metrics *agent.JVMMetrics) (*agent.Downstream, error) {
	...
	if !ok {
		logger.Warning(j.context.GetRuntimeContext(), selfmonitor.SkywalkingResourceNotReady, "failed to find application instance registry info", "applicationInstanceId", metrics.ApplicationInstanceId)
