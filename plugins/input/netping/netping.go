...
func (m *NetPing) Init(context pipeline.Context) (int, error) {
	...
	for _, c := range m.HTTPConfigs {
		...
		if c.Src == m.ip {
			...
			if err != nil {
				logger.Error(context.GetRuntimeContext(), util.PLUGIN_INIT_ALARM, "netping failed to parse httping target")
				continue
			}

			if u.Host == "" {
				logger.Error(context.GetRuntimeContext(), util.PLUGIN_INIT_ALARM, "netping failed to parse httping target, get empty host")
				continue
			}
...
func (m *NetPing) doICMPing(config *ICMPConfig) {
	...
	if err != nil {
		logger.Error(m.context.GetRuntimeContext(), util.PLUGIN_RUNTIME_ALARM, err.Error())
