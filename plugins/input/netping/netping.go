...
func (m *NetPing) Init(context pipeline.Context) (int, error) {
	...
	for _, c := range m.HTTPConfigs {
		...
		if c.Src == m.ip {
			...
			if err != nil {
				logger.Error(context.GetRuntimeContext(), util.InputInitAlarm, "netping failed to parse httping target")
				continue
			}

			if u.Host == "" {
				logger.Error(context.GetRuntimeContext(), util.InputInitAlarm, "netping failed to parse httping target, get empty host")
				continue
			}
...
func (m *NetPing) doICMPing(config *ICMPConfig) {
	...
	if err != nil {
		logger.Error(m.context.GetRuntimeContext(), util.InputCollectAlarm, "fail to init ping", err.Error())
	...
	if err != nil {
		logger.Error(m.context.GetRuntimeContext(), util.InputCollectAlarm, "fail to run ping", err.Error())
		label.Append("err", err.Error())
		m.resultChannel <- &Result{
			Valid:  true,
			Type:   PingTypeIcmp,
...
func (m *NetPing) doHTTPing(config *HTTPConfig) {
	...
	if err != nil {
		logger.Error(m.context.GetRuntimeContext(), util.InputCollectAlarm, "fail to run httping", err.Error())
		label.Append("err", err.Error())
		m.resultChannel <- &Result{
			Valid:   true,
			Type:    PingTypeHttping,
