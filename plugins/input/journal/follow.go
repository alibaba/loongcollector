		...
		for {
			...
			if err != nil && err != io.EOF {
				if cursor, cerr := journal.GetCursor(); cerr != nil {
					logger.Warningf(sj.context.GetRuntimeContext(), util.PLUGIN_RUNTIME_ALARM, "Received unknown error when reading a new entry: %v, cursor read error: %v", err, cerr)
				} else {
					logger.Warningf(sj.context.GetRuntimeContext(), util.PLUGIN_RUNTIME_ALARM, "Received unknown error when reading a new entry: %v, cursor: %s", err, cursor)
				}
			...
			for {
				...
				case e := <-eventWaitCh:
					...
					case -9:
						continue process
					default:
						logger.Warningf(sj.context.GetRuntimeContext(), util.PLUGIN_RUNTIME_ALARM, "Received unknown event: %d", e)
						util.RandomSleep(time.Second*5, 0.1, stop)
					}
				}
			}
