...
func (sj *ServiceJournal) SaveCheckpoint(forceFlag bool) {
	...
	if cursor, err := sj.journal.GetCursor(); err != nil {
		logger.Error(sj.context.GetRuntimeContext(), util.CHECKPOINT_ALARM, "get cursor error", err)
	} else if err := sj.context.SaveCheckPoint(checkPointKey, []byte(cursor)); err != nil {
		logger.Error(sj.context.GetRuntimeContext(), util.CHECKPOINT_ALARM, "save checkpoint error", err)
	}
}
...
func (sj *ServiceJournal) initJournal() error {
	...
	seekToHelper := func(position string, err error) error {
		if err == nil {
			...
			logger.Warningf(sj.context.GetRuntimeContext(), util.PLUGIN_START_ALARM, "Could not seek to %s: %v", position, err)
		}
		return err
	}
	...
	if len(sj.lastCPCursor) == 0 {
		...
		if err != nil {
			logger.Warning(sj.context.GetRuntimeContext(), util.PLUGIN_START_ALARM, "call next when init error", err)
			err = nil
		}
	}
	if err != nil {
