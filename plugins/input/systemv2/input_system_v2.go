	...
	if err == nil && len(cpuStat) > 0 {
		...
		if len(cpushareEnv) > 0 {
			...
			if err != nil || cpuRequest <= 0 || ncpus == 0 {
				logger.Error(r.context.GetRuntimeContext(), util.PLUGIN_RUNTIME_ALARM, "get sigma env failed",
					"error", err,
					"ncpus", ncpus,
...
func (r *InputSystem) Collect(collector pipeline.Collector) error {
	...
	if r.Disk {
		...
		if err != nil {
			logger.Error(r.context.GetRuntimeContext(), util.PLUGIN_RUNTIME_ALARM, "read disk metrics timeout, would skip disk collection", err)
			r.Disk = false
		}
	}
	if r.Net {
