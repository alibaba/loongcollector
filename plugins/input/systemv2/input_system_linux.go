...
func (r *InputSystem) Init(context pipeline.Context) (int, error) {
	...
	if err != nil {
		logger.Error(r.context.GetRuntimeContext(), util.PLUGIN_INIT_ALARM, "err", err)
		return 0, err
	}
	r.fs = &fs
...
func (r *InputSystem) CollectTCPStats(collector pipeline.Collector, stat *net.ProtoCountersStat) {
	...
	if errTCP != nil {
		logger.Error(r.context.GetRuntimeContext(), util.PLUGIN_RUNTIME_ALARM, "err", errTCP)
	}
	tcp6, errTCP6 := r.fs.NetTCP6()
	if errTCP6 != nil {
		logger.Error(r.context.GetRuntimeContext(), util.PLUGIN_RUNTIME_ALARM, "err", errTCP6)
	}
...
func (r *InputSystem) CollectOpenFD(collector pipeline.Collector) {
	...
	if err != nil {
		logger.Error(r.context.GetRuntimeContext(), util.PLUGIN_RUNTIME_ALARM, "err", err)
		return
	}
	...
	if len(parts) < 3 {
		logger.Error(r.context.GetRuntimeContext(), util.PLUGIN_RUNTIME_ALARM, "want", 3, "got", len(parts))
		return
	}
...
func (r *InputSystem) CollectDiskUsage(collector pipeline.Collector) {
	...
	for scanner.Scan() {
		...
		if len(parts) < 4 {
			logger.Error(r.context.GetRuntimeContext(), util.PLUGIN_RUNTIME_ALARM, "got", text)
			return
		}
