...
func (r *InputSystem) Init(context pipeline.Context) (int, error) {
	...
	if err != nil {
		logger.Error(r.context.GetRuntimeContext(), util.InputInitAlarm, "read proc err", err)
		return 0, err
	}
	r.fs = &fs
...
func (r *InputSystem) CollectTCPStats(collector pipeline.Collector, stat *net.ProtoCountersStat) {
	...
	if errTCP != nil {
		logger.Error(r.context.GetRuntimeContext(), util.InputCollectAlarm, "read proc tcp err", errTCP)
	}
	tcp6, errTCP6 := r.fs.NetTCP6()
	if errTCP6 != nil {
		logger.Error(r.context.GetRuntimeContext(), util.InputCollectAlarm, "read proc tcp6 err", errTCP6)
	}
...
func (r *InputSystem) CollectOpenFD(collector pipeline.Collector) {
	...
	if err != nil {
		logger.Error(r.context.GetRuntimeContext(), util.InputCollectAlarm, "open /proc/sys/fs/file-nr err", err)
		return
	}
	...
	if err != nil {
		logger.Error(r.context.GetRuntimeContext(), util.InputCollectAlarm, "read /proc/sys/fs/file-nr err", err)
		return
	}
	parts := bytes.Split(bytes.TrimSpace(content), []byte("\u0009"))
	if len(parts) < 3 {
		logger.Error(r.context.GetRuntimeContext(), util.InputCollectAlarm, "/proc/sys/fs/file-nr pattern error, want", 3, "got", len(parts))
		return
	}
...
func (r *InputSystem) CollectDiskUsage(collector pipeline.Collector) {
	...
	if err != nil {
		logger.Error(r.context.GetRuntimeContext(), util.InputCollectAlarm, "open /proc/1/mounts err", err)
		return
	}
	defer func(file *os.File) {
		_ = file.Close()
	...
	for scanner.Scan() {
		...
		if len(parts) < 4 {
			logger.Error(r.context.GetRuntimeContext(), util.InputCollectAlarm, "read /proc/1/mounts err, got", text)
			return
		}
