...
func (r *InputSystem) Init(context pipeline.Context) (int, error) {
	...
	fs, err := procfs.NewFS(dockercenter.GetMountedFilePath(procfs.DefaultMountPoint))
	if err != nil {
...
func (r *InputSystem) CollectOpenFD(collector pipeline.Collector) {
	// mount the host proc path
	file, err := os.Open(dockercenter.GetMountedFilePath("/proc/sys/fs/file-nr"))
	if err != nil {
...
func (r *InputSystem) CollectDiskUsage(collector pipeline.Collector) {
	// mount the host proc path
	file, err := os.Open(dockercenter.GetMountedFilePath("/proc/1/mounts"))
	if err != nil {
	...
	for scanner.Scan() {
		...
		usage, err := disk.Usage(dockercenter.GetMountedFilePath(parts[1]))
		if err == nil {
