...
func findAllProcessCache(maxLabelLength int) ([]processCache, error) {
	...
	if err != nil {
		logger.Errorf(context.Background(), util.PLUGIN_RUNTIME_ALARM, "error: %v", err)
		return nil, err
	}
	if len(processes) == 0 {
	...
	if pc.meta.labels == nil {
		if name := pc.getCommName(); name != "" {
			...
			if pc.meta.maxLabelLength < len(name) {
				logger.Warningf(context.Background(), util.PLUGIN_RUNTIME_ALARM, "the stat cmdline label is over %d chars: %s", pc.meta.maxLabelLength, name)
