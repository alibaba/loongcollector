...
func findAllProcessCache(maxLabelLength int) ([]processCache, error) {
	...
	if err != nil {
		logger.Error(context.Background(), util.InputCollectAlarm, "open process error", err)
		return nil, err
	}
	procs, err := fs.AllProcs()
	if err != nil {
		logger.Error(context.Background(), util.InputCollectAlarm, "process listen error", err)
		return nil, err
	}
	if len(procs) == 0 {
...
func (pc *processCacheLinux) Labels(customLabels *helper.MetricLabels) *helper.MetricLabels {
	if pc.meta.labels == nil {
		...
		if pc.meta.maxLabelLength < len(comm) {
			logger.Warningf(context.Background(), util.InputCollectAlarm, "process label too long, the stat comm label is over %d chars: %s", pc.meta.maxLabelLength, comm)
