...
func (p *LogCanal) Fire(e *canalLog.Entry) error {
	...
	case e.Level == canalLog.WarnLevel:
		logger.Warning(context.Background(), util.PLUGIN_RUNTIME_ALARM, "canal log, level", e.Level.String(), "message", e.Message)
	case e.Level == canalLog.ErrorLevel || e.Level == canalLog.FatalLevel || e.Level == canalLog.PanicLevel:
		logger.Error(context.Background(), util.PLUGIN_RUNTIME_ALARM, "canal log, level", e.Level.String(), "message", e.Message)
	default:
...
ValueLoop:
	...
	marshal := func(fieldName string, value map[string]string) {
		...
		logger.Warningf(sc.context.GetRuntimeContext(), util.PLUGIN_RUNTIME_ALARM,
...
func (sc *ServiceCanal) OnRow(e *canal.RowsEvent) error {
	...
	if e.Action == canal.UpdateAction {
		...
		if len(e.Rows)%2 != 0 {
			logger.Error(sc.context.GetRuntimeContext(), util.PLUGIN_RUNTIME_ALARM, "invalid update value count", len(e.Rows))
			return nil
		}
			...
			if len(e.Rows[i]) != len(e.Table.Columns) || len(e.Rows[i+1]) != len(e.Table.Columns) {
				...
				if i == 0 && !(len(e.Rows[i])-len(e.Table.Columns) == 1 && len(e.Table.PKColumns) == 0) {
					...
					if err != nil || tableMeta == nil {
						logger.Error(sc.context.GetRuntimeContext(), util.PLUGIN_RUNTIME_ALARM, "invalid row values", e.Table.Name,
...
func (sc *ServiceCanal) GetBinlogLatestPos() mysql.Position {
	...
	if err == nil && result != nil {
		if len(result.Values) > 0 {
			...
			if len(value) == 2 {
				...
				if errConv != nil {
					...
					logger.Error(sc.context.GetRuntimeContext(), util.PLUGIN_RUNTIME_ALARM, "show binary logs error")
				}
		...
		logger.Error(sc.context.GetRuntimeContext(), util.PLUGIN_RUNTIME_ALARM, "show binary logs error", err)
	}
...
func (sc *ServiceCanal) newCanal() (bool, error) {
	...
	for {
		...
		logger.Warning(sc.context.GetRuntimeContext(), util.PLUGIN_START_ALARM, "newCanal failed, error", err, "retry it")
...
func (sc *ServiceCanal) Start(c pipeline.Collector) error {
	...
	if err != nil {
		logger.Error(sc.context.GetRuntimeContext(),
			util.PLUGIN_START_ALARM, "service_canal plugin only supports ROW mode", err)
		return err
	}
	if shouldShutdown {
	...
	for {
		...
		logger.Warning(sc.context.GetRuntimeContext(), util.PLUGIN_START_ALARM, err.Error())
		if shouldRetry {
			if util.RandomSleep(time.Second*5, 0.1, sc.shutdown) {
				sc.canal.Close()
	...
	if sc.checkpoint.GTID != "" {
		...
		if err != nil {
			logger.Error(sc.context.GetRuntimeContext(), util.PLUGIN_START_ALARM, "Parse GTID error, clear it",
	...
	if nil == gtid && 0 == len(startPos.Name) && !sc.StartFromBegining {
		if sc.isGTIDEnabled {
			...
			if err != nil {
				logger.Warning(sc.context.GetRuntimeContext(), util.PLUGIN_START_ALARM, "Call getLatestGTID failed, error", err)
			}
		}
		if gtid == nil {
...
ForBlock:
	for {
		...
		case err = <-sc.lastErrorChan:
			...
			errStr := err.Error()
			logger.Error(sc.context.GetRuntimeContext(), util.PLUGIN_RUNTIME_ALARM, "Restart canal because of error", err)
			...
			if strings.Contains(errStr, "ERROR 1236") {
				...
				if sc.isGTIDEnabled {
					gtid, err = sc.getLatestGTID()
					if err != nil {
						logger.Warning(sc.context.GetRuntimeContext(), util.PLUGIN_RUNTIME_ALARM,
