...
func (in *InputKubernetesMeta) Init(context pipeline.Context) (int, error) {
	...
	if in.IntervalMs < 5000 {
		logger.Warning(in.context.GetRuntimeContext(), selfmonitor.KubernetesMetaFetchIntervalAlarm, "interval", "must over than 5000 ms")
	...
	if in.LabelSelectors == "" {
		...
		if err != nil {
			logger.Warning(in.context.GetRuntimeContext(), selfmonitor.KubernetesLabelSelectorError, "error", err)
...
func (in *InputKubernetesMeta) Collect(collector pipeline.Collector) error {
	...
	for _, c := range in.collectors {
		...
		if err != nil {
			logger.Warning(in.context.GetRuntimeContext(), selfmonitor.KubernetesMetaCollectError, "error", err)
			continue
		}
		if len(nodes) == 0 {
			continue
