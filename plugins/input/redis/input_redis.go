...
func (r *InputRedis) Collect(collector pipeline.Collector) error {
	...
	for _, serv := range r.ServerUrls {
		...
		if err != nil {
			logger.Warning(r.context.GetRuntimeContext(), util.RedisParseAddressAlarm, "Unable to parse to address", serv, "error", err)
			continue
		} else if u.Scheme == "" {
		...
		go func(url *url.URL) {
			...
			if err != nil {
				logger.Error(r.context.GetRuntimeContext(), util.InputCollectAlarm, "redis collect fail", err)
			}
		}(u)
	}
