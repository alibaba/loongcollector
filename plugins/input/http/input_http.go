...
func (h *Response) httpGather(address string) (map[string]string, error) {
	...
	if err != nil {
		logger.Error(h.context.GetRuntimeContext(), util.HttpParseAlarm, "Read body of HTTP response failed", err)
	...
	if h.ResponseStringMatch != "" {
		...
		if h.compiledStringMatch == nil {
			...
			if err != nil {
				logger.Error(h.context.GetRuntimeContext(), util.HttpInitAlarm, "Compile regular expression faild", h.ResponseStringMatch, "error", err)
...
func (h *Response) Collect(collector pipeline.Collector) error {
	...
	if len(h.AddressPath) > 0 {
		...
		if curTime.Sub(h.lastLoadAddressTime).Seconds() > float64(h.FlushAddressIntervalSec) {
			err := h.loadAddresses()
			if err != nil {
				logger.Warning(h.context.GetRuntimeContext(), util.HttpLoadAddressAlarm, "load address error, file", h.AddressPath, "error", err)
			}
		}
	}
	...
	for _, address := range h.Addresses {
		...
		if err != nil {
			logger.Warning(h.context.GetRuntimeContext(), util.HttpCollectAlarm, "collect error, address", address, "error", err)
		}
		if len(fields) > 0 {
			// Add metrics
