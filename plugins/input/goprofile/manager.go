...
func (m *Manager) Start(p *GoProfile) error {
	...
	go func() {
		...
		if err := m.discoveryManager.ApplyConfig(map[string]discovery.Configs{
			...
			logger.Error(context.Background(), util.InputCollectAlarm, "apply discovery config error", err.Error())
		}
		err := m.discoveryManager.Run()
		logger.Debug(context.Background(), "finish discovery manager")
		if err != nil {
			logger.Error(context.Background(), util.InputCollectAlarm, "discovery err", err.Error())
		}
	}()
	go func() {
		...
		if err != nil {
			logger.Error(context.Background(), util.InputCollectAlarm, "scrape err", err.Error())
		}
	}()
	return nil
}
