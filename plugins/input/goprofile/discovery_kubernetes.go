...
func (k *KubernetesConfig) InitDiscoverer() {
	...
	var err error
	k.IncludeEnv, k.includeEnvRegex, err = dockercenter.SplitRegexFromMap(k.IncludeEnv)
	if err != nil {
		logger.Warning(context.Background(), "INVALID_REGEX_ALARM", "init include env regex error", err)
	}
	k.ExcludeEnv, k.excludeEnvRegex, err = dockercenter.SplitRegexFromMap(k.ExcludeEnv)
	if err != nil {
		logger.Warning(context.Background(), "INVALID_REGEX_ALARM", "init exclude env regex error", err)
	}
	k.IncludeContainerLabel, k.includeContainerLabelRegex, err = dockercenter.SplitRegexFromMap(k.IncludeContainerLabel)
	if err != nil {
		logger.Warning(context.Background(), "INVALID_REGEX_ALARM", "init include label regex error", err)
	}
	k.ExcludeContainerLabel, k.excludeContainerLabelRegex, err = dockercenter.SplitRegexFromMap(k.ExcludeContainerLabel)
	if err != nil {
		logger.Warning(context.Background(), "INVALID_REGEX_ALARM", "init exclude label regex error", err)
	}
	k.k8sFilter, err = dockercenter.CreateK8SFilter(k.K8sNamespaceRegex, k.K8sPodRegex, k.K8sContainerRegex, k.IncludeK8sLabel, k.ExcludeK8sLabel)
	if err != nil {
...
func (k *KubernetesConfig) Run(ctx context.Context, up chan<- []*targetgroup.Group) {
	...
	for {
		...
		case <-ctx.Done():
			return
		case <-ticker.C:
			containers := dockercenter.GetContainerByAcceptedInfo(k.IncludeContainerLabel, k.ExcludeContainerLabel,
...
func (k *KubernetesConfig) convertContainers2Group(containers map[string]*dockercenter.DockerInfoDetail) []*targetgroup.Group {
