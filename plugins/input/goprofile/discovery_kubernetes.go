...
func (k *KubernetesConfig) InitDiscoverer() {
	...
	if err != nil {
		logger.Warning(context.Background(), util.PLUGIN_INIT_ALARM, "init include env regex error", err)
	}
	k.ExcludeEnv, k.excludeEnvRegex, err = helper.SplitRegexFromMap(k.ExcludeEnv)
	if err != nil {
		logger.Warning(context.Background(), util.PLUGIN_INIT_ALARM, "init exclude env regex error", err)
	}
	k.IncludeContainerLabel, k.includeContainerLabelRegex, err = helper.SplitRegexFromMap(k.IncludeContainerLabel)
	if err != nil {
		logger.Warning(context.Background(), util.PLUGIN_INIT_ALARM, "init include label regex error", err)
	}
	k.ExcludeContainerLabel, k.excludeContainerLabelRegex, err = helper.SplitRegexFromMap(k.ExcludeContainerLabel)
	if err != nil {
		logger.Warning(context.Background(), util.PLUGIN_INIT_ALARM, "init exclude label regex error", err)
	}
	...
	if err != nil {
		logger.Warning(context.Background(), util.PLUGIN_INIT_ALARM, "init k8s filter error", err)
	}
