...
func (k *KubernetesConfig) InitDiscoverer() {
	...
	if err != nil {
		logger.Warning(context.Background(), selfmonitor.InvalidRegexAlarm, "init include env regex error", err)
	}
	k.ExcludeEnv, k.excludeEnvRegex, err = containercenter.SplitRegexFromMap(k.ExcludeEnv)
	if err != nil {
		logger.Warning(context.Background(), selfmonitor.InvalidRegexAlarm, "init exclude env regex error", err)
	}
	k.IncludeContainerLabel, k.includeContainerLabelRegex, err = containercenter.SplitRegexFromMap(k.IncludeContainerLabel)
	if err != nil {
		logger.Warning(context.Background(), selfmonitor.InvalidRegexAlarm, "init include label regex error", err)
	}
	k.ExcludeContainerLabel, k.excludeContainerLabelRegex, err = containercenter.SplitRegexFromMap(k.ExcludeContainerLabel)
	if err != nil {
		logger.Warning(context.Background(), selfmonitor.InvalidRegexAlarm, "init exclude label regex error", err)
	}
	...
	if err != nil {
		logger.Warning(context.Background(), selfmonitor.InvalidRegexAlarm, "init k8s filter error", err)
	}
