...
func (p *KubernetesMeta) readKubernetesWorkloadMeta() bool {
	...
	if err != nil {
		logger.Error(p.context.GetRuntimeContext(), util.PLUGIN_RUNTIME_ALARM, "read namespace err", err)
		return false
	}
	config, err := rest.InClusterConfig()
	if err != nil {
		logger.Error(p.context.GetRuntimeContext(), util.PLUGIN_RUNTIME_ALARM, "create in cluster config err", err)
		return false
	}
	...
	if p.currentNum = helper.ExtractStatefulSetNum(podName); p.currentNum == -1 {
		logger.Error(p.context.GetRuntimeContext(), util.PLUGIN_RUNTIME_ALARM, "read current num err", p.workloadName)
		return false
	}
	if p.workloadName = helper.ExtractPodWorkload(podName); p.workloadName == "" {
		logger.Error(p.context.GetRuntimeContext(), util.PLUGIN_RUNTIME_ALARM, "read workload name err", p.workloadName)
		return false
	}
	for i := 0; i < 3; i++ {
	...
	if err != nil {
		logger.Error(p.context.GetRuntimeContext(), util.PLUGIN_RUNTIME_ALARM, "read replicas err", err)
...
func (p *KubernetesMeta) getPrometheusReplicas() (change bool, err error) {
	...
	if err != nil {
		...
		if p.readReplicasErrCount >= 30 {
			logger.Error(p.context.GetRuntimeContext(), util.PLUGIN_RUNTIME_ALARM, "cannot get kubernetes ilogtail cluster mode replicas", err)
