...
func (m *metaCollector) handleEvent(event []*k8smeta.K8sMetaEvent) {
	...
	default:
		logger.Error(context.Background(), util.InputCollectAlarm, "unknown event type", event[0].EventType)
	}
}
...
func (m *metaCollector) processEntityJSONObject(obj interface{}) string {
	...
	if err != nil {
		logger.Error(context.Background(), util.InputCollectAlarm, "process entity json object fail", err)
		return "{}"
	}
...
func (m *metaCollector) processEntityJSONArray(obj []map[string]string) string {
	...
	objStr, err := json.Marshal(obj)
	if err != nil {
		logger.Error(context.Background(), util.InputCollectAlarm, "process entity json array fail", err)
		return "[]"
	}
	return string(objStr)
}
...
func (m *metaCollector) send(event models.PipelineEvent, entity bool) {
	...
	case <-time.After(3 * time.Second):
		logger.Error(context.Background(), util.InputCollectAlarm, "send event timeout, isEntity", entity)
	}
}
		...
		for k, v := range modelLog.Contents.Iterator() {
			if _, ok := v.(string); !ok {
				if intValue, ok := v.(int); !ok {
					logger.Error(context.Background(), util.InputCollectAlarm, "convert event to log fail, value is not string", v, "key", k)
					continue
				} else {
