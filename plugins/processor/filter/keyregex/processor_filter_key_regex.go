...
func (p *ProcessorKeyFilter) Init(context pipeline.Context) error {
	...
	if len(p.Include) > 0 {
		...
		for key, val := range p.Include {
			...
			if err != nil {
				logger.Warning(p.context.GetRuntimeContext(), util.FilterInitAlarm, "init include filter error, key", key, "regex", val, "error", err)
				return err
			}
	...
	if len(p.Exclude) > 0 {
		...
		for key, val := range p.Exclude {
			reg, err := regexp.Compile(val)
			if err != nil {
				logger.Warning(p.context.GetRuntimeContext(), util.FilterInitAlarm, "init exclude filter error, key", key, "regex", val, "error", err)
				return err
			}
