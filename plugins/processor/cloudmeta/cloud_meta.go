...
func (c *ProcessorCloudMeta) Init(context pipeline.Context) error {
	...
	if m == nil {
		...
		logger.Error(c.context.GetRuntimeContext(), util.PLUGIN_INIT_ALARM, "not support platform", c.Platform)
	} else {
		c.manager = m
		c.manager.StartCollect()
	}
	if len(c.Metadata) == 0 {
		logger.Error(c.context.GetRuntimeContext(), util.PLUGIN_INIT_ALARM, "metadata is required")
...
func (c *ProcessorCloudMeta) ProcessLog(log *protocol.Log) {
	...
	if err := json.Unmarshal(util.ZeroCopyStringToBytes(content.Value), &res); err != nil {
		logger.Warning(c.context.GetRuntimeContext(), util.PARSE_LOG_FAIL_ALARM, "json deserialize err", err)
		return
	}
