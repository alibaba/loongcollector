...
func (p *ProcessorRegex) Init(context pipeline.Context) error {
	...
	if err != nil {
		logger.Error(p.context.GetRuntimeContext(), util.PLUGIN_INIT_ALARM, "init regex error", err, "regex", p.Regex)
		return err
	}
...
func (p *ProcessorRegex) processRegex(log *protocol.Log, val *string) bool {
	...
	if len(indexArray) < 2 || (p.FullMatch && (indexArray[0] != 0 || indexArray[1] != len(*val))) {
		if p.NoMatchError {
			logger.Warning(p.context.GetRuntimeContext(), util.REGEX_MATCH_ALARM, "unmatch this log content", util.CutString(*val, 512))
		}
		return false
	}
	...
	if (len(indexArray)>>1 - 1) < len(p.Keys) {
		if p.NoMatchError {
			logger.Warning(p.context.GetRuntimeContext(), util.REGEX_MATCH_ALARM, "match result count less than key count, result count", len(indexArray)>>1-1, "key count", len(p.Keys))
		}
		return false
	}
