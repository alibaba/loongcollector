...
func (p *ProcessorSplitChar) splitValue(log *protocol.Log, value string) bool {
				...
				for newValue := []byte{}; i < len(value); i++ {
					if value[i] == p.quoteChar {
						...
						if value[i+1] == p.quoteChar { // Case 3.
							...
							logger.Warning(p.context.GetRuntimeContext(), util.PARSE_LOG_FAIL_ALARM, "no continue quote", p.quoteChar, "log", util.CutString(value, 1024))
							return false
						}
					} else {
						newValue = append(newValue, value[i])
	...
	if keyIndex < len(p.SplitKeys) && p.NoMatchError {
		logger.Warning(p.context.GetRuntimeContext(), util.PARSE_LOG_FAIL_ALARM, "split keys not match, split len", keyIndex, "log", util.CutString(value, 1024))
	}
	return true
}
