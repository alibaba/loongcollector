...
func (p *ProcessorPackjson) processLog(log *protocol.Log) {
	...
	if p.AlarmIfIncomplete && len(p.SourceKeys) != len(packMap) {
		...
		logger.Warningf(p.context.GetRuntimeContext(), util.ProcessorProcessAlarm, "processor_packjson SourceKeys not found %v", emptyKeys)
	}
	newValue, err := json.Marshal(packMap)
	if err != nil {
		logger.Errorf(p.context.GetRuntimeContext(), util.ProcessorProcessAlarm, "processor_packjson package json error %v packMap: %v", err, packMap)
		return
	}
