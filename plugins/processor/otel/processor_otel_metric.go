...
func (p *ProcessorOtelMetricParser) Init(context pipeline.Context) error {
	p.context = context
	if p.Format == "" {
		logger.Warningf(p.context.GetRuntimeContext(), util.PARSE_LOG_FAIL_ALARM, "data format is empty, use protobuf")
...
func (p *ProcessorOtelMetricParser) ProcessLogs(logArray []*protocol.Log) []*protocol.Log {
	...
	for _, log := range logArray {
		if l, err := p.processLog(log); err != nil {
			logger.Errorf(p.context.GetRuntimeContext(), util.PARSE_LOG_FAIL_ALARM, "parser otel trace error %v", err)
		} else {
			logs = append(logs, l...)
		}
	}
