...
func (p *ProcessorCSVDecoder) decodeCSV(log *protocol.Log, value string) bool {
	...
	if err != nil && err != io.EOF {
		logger.Warning(p.context.GetRuntimeContext(), util.ProcessorProcessAlarm, "cannot decode log", err, "log", util.CutString(value, 1024))
		return false
	}
	...
	if len(p.SplitKeys) != len(record) {
		logger.Warning(p.context.GetRuntimeContext(), util.ProcessorProcessAlarm, "decode keys not match, split len", len(record), "log", util.CutString(value, 1024))
	}
	return true
}
