	...
	for i := range p.Switch {
		...
		default:
			if len(relationOpertor) > 0 {
				logger.Warning(p.context.GetRuntimeContext(), util.PLUGIN_INIT_ALARM, "init relationOpertor error, relationOpertor", relationOpertor)
			}
		...
		default:
			if len(logicalOperator) > 0 {
				logger.Warning(p.context.GetRuntimeContext(), util.PLUGIN_INIT_ALARM, "init logicalOperator error, logicalOperator", logicalOperator)
			}
		...
		if p.Switch[i].Case.FieldConditions != nil {
			...
			for key, val := range p.Switch[i].Case.FieldConditions {
				...
				case RelationOperatorRegexp:
					...
					if err != nil {
						logger.Warning(p.context.GetRuntimeContext(), util.PLUGIN_INIT_ALARM, "init condition regex error, key", key, "regex", val, "error", err)
						return err
					}
		...
		if p.Switch[i].Actions != nil {
			...
			for j := range p.Switch[i].Actions {
				...
				if typeName, ok := action["type"]; ok {
					...
					default:
						logger.Error(p.context.GetRuntimeContext(), util.PLUGIN_INIT_ALARM, "init condition action type error, type", typeName)
