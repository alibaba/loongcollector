...
func (p *ProcessorJSON) processLog(log *protocol.Log) {
	...
	for idx := range log.Contents {
		if log.Contents[idx].Key == p.SourceKey {
			...
			if err != nil {
				logger.Errorf(p.context.GetRuntimeContext(), util.ParseLogFailAlarm, "parser json error %v", err)
			}
	...
	if !findKey && p.NoKeyError {
		logger.Warningf(p.context.GetRuntimeContext(), util.ParseLogFailAlarm, "cannot find key %v", p.SourceKey)
	}
}
...
func (p *ProcessorJSON) processEvent(event models.PipelineEvent) {
	...
	if !ok {
		logger.Warningf(p.context.GetRuntimeContext(), util.ParseLogFailAlarm, "key %v is not string", p.SourceKey)
		return
	}
	param := ExpandParam{
		sourceKey:            p.SourceKey,
