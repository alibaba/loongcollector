...
func (p *ProcessorGrok) Init(context pipeline.Context) error {
	...
	if len(p.CustomPatternDir) > 0 {
		for _, path := range p.CustomPatternDir {
			...
			if err != nil {
				logger.Error(p.context.GetRuntimeContext(), util.PLUGIN_INIT_ALARM, "init grok's custom pattern in dir error", err)
				return err
			}
		}
	}
	...
	err = p.buildPatterns()
	if err != nil {
		logger.Error(p.context.GetRuntimeContext(), util.PLUGIN_INIT_ALARM, "build grok's pattern error", err)
		return err
	}

	err = p.compileMatchs()
	if err != nil {
		logger.Error(p.context.GetRuntimeContext(), util.PLUGIN_INIT_ALARM, "compile grok's matchs error", err)
		return err
	}
	...
	for i, cont := range log.Contents {
		if len(p.SourceKey) == 0 || p.SourceKey == cont.Key {
			...
			if parseResult == matchFail && p.NoMatchError {
				logger.Warning(p.context.GetRuntimeContext(), util.PARSE_LOG_FAIL_ALARM, "all match fail", p.SourceKey, cont.Value)
			}

			// tome out error
			if parseResult == matchTimeOut && p.TimeoutError {
				logger.Warning(p.context.GetRuntimeContext(), util.PARSE_LOG_FAIL_ALARM, "match time out", p.SourceKey, cont.Value)
			}
