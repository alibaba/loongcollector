...
func (p *ProcessorGotime) processLog(log *protocol.Log) {
		...
		if content.Key == p.SourceKey {
			...
			if p.timestampFormat {
				...
				if err != nil && p.AlarmIfFail {
					logger.Warningf(p.context.GetRuntimeContext(), util.PARSE_TIME_FAIL_ALARM, "ParseInt(%v, %v) failed: %v",
				...
				if err != nil && p.AlarmIfFail {
					logger.Warningf(p.context.GetRuntimeContext(), util.PARSE_TIME_FAIL_ALARM, "ParseInLocation(%v, %v, %v) failed: %v",
	...
	if !found && p.NoKeyError {
		logger.Warningf(p.context.GetRuntimeContext(), util.PARSE_TIME_FAIL_ALARM, "cannot find key %v", p.SourceKey)
	}
}
