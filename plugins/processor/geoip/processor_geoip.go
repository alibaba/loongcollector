...
func (p *ProcessorGeoIP) ProcessLog(log *protocol.Log) {
	...
	if !findKey && p.NoKeyError {
		logger.Warning(p.context.GetRuntimeContext(), util.GeoipAlarm, "cannot find key", p.SourceKey)
	}
}
...
func (p *ProcessorGeoIP) ProcessGeoIP(log *protocol.Log, val *string) {
	...
	if ip == nil {
		if p.NoMatchError {
			logger.Warning(p.context.GetRuntimeContext(), util.GeoipAlarm, "invalid ip", *val)
		}
		return
	}
	record, err := p.db.City(ip)
	if err != nil && p.NoMatchError {
		logger.Warning(p.context.GetRuntimeContext(), util.GeoipAlarm, "parse ip", ip, "error", err)
		return
	}
