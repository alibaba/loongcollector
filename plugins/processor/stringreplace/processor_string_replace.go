...
func (p *ProcessorStringReplace) Init(context pipeline.Context) error {
	...
	case MethodRegex:
		...
		if err != nil {
			logger.Error(p.context.GetRuntimeContext(), util.PLUGIN_INIT_ALARM, "init regex error", err, "regex", p.Match)
			return err
		}
	case MethodUnquote:
	default:
...
func (p *ProcessorStringReplace) ProcessLogs(logArray []*protocol.Log) []*protocol.Log {
	...
	for _, log := range logArray {
		for _, cont := range log.Contents {
			...
			if err != nil {
				logger.Error(p.context.GetRuntimeContext(), util.PLUGIN_INIT_ALARM, "process log error", err)
				newContVal = cont.Value
			}
			if len(p.DestKey) > 0 {
