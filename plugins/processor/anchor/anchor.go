...
func (p *ProcessorAnchor) ProcessLog(log *protocol.Log) {
	...
	if !findKey && p.NoKeyError {
		logger.Warning(p.context.GetRuntimeContext(), util.AnchorFindAlarm, "anchor cannot find key", p.SourceKey)
	}
...
func (p *ProcessorAnchor) ProcessAnchor(log *protocol.Log, val *string) {
	for _, anchor := range p.Anchors {
		...
		if startIndex < 0 {
			if p.NoAnchorError {
				logger.Warning(p.context.GetRuntimeContext(), util.AnchorFindAlarm, "anchor no start", anchor.Start, "content", util.CutString(*val, 1024))
			}
			continue
		}
		...
		if stopIndex < 0 {
			if p.NoAnchorError {
				logger.Warning(p.context.GetRuntimeContext(), util.AnchorFindAlarm, "anchor no stop", anchor.Stop, "content", util.CutString(*val, 1024))
			}
			continue
		} else {
		...
		case JSONType:
			...
			if err != nil && !anchor.IgnoreJSONError {
				logger.Warning(p.context.GetRuntimeContext(), util.AnchorJsonAlarm, "process json error", err, "content", (*val)[startIndex:stopIndex])
			}
		}
	}
}
