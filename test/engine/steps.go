...
func ScenarioInitializer(ctx *godog.ScenarioContext) {
	...
	ctx.Given(`^loongcollector container environment \{(.*)\} \{(.*)\}$`, setup.SetContainerEnvironment)
	ctx.Given(`^loongcollector expose port \{(.*)\} to \{(.*)\}`, setup.ExposePort)
	ctx.Given(`^\{(.*)\} local config as below`, control.AddLocalConfig)
	ctx.Given(`^remove local config \{(.*)\}`, control.RemoveLocalConfig)
	...
	ctx.When(`^\{(.*)\} local config as below`, control.AddLocalConfig)
	...
	ctx.When(`^start monitor \{(\S+)\}, with timeout \{(\d+)\} min$`, monitor.StartMonitor)
	...
	ctx.Then(`^collected log field \{(.*)\} has no duplicates$`, verify.LogFieldNoDuplicates)
	...
	ctx.Then(`^the kafka partitions consistent by field \{(\S+)\}$`, verify.KafkaPartitionsConsistentByField)
	ctx.Then(`^print kafka partition mapping by field \{(\S+)\}$`, verify.KafkaPrintPartitionMappingByField)
