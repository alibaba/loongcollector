...
func (i *InfluxdbSubscriber) GetData(_ string, startTime int32) ([]*protocol.LogGroup, error) {
	...
	if i.CreateDb {
		...
		if err != nil {
			logger.Warningf(context.Background(), selfmonitor.InfluxdbSubscriberAlarm, "failed to create database %s, err: %s", i.DbName, err)
		}
	}
	...
	if err != nil {
		logger.Warning(context.Background(), selfmonitor.InfluxdbSubscriberAlarm, "err", err)
		return nil, err
	}
