...
func (p *pluginv2Runner) addAggregator(pluginMeta *pipeline.PluginMeta, aggregator pipeline.AggregatorV2) error {
	...
	err := wrapper.Init(pluginMeta)
	if err != nil {
		logger.Error(p.LogstoreConfig.Context.GetRuntimeContext(), util.PLUGIN_INIT_ALARM, "Aggregator failed to initialize", aggregator.Description(), "error", err)
		return err
	}
	...
	for _, input := range p.ServicePlugins {
		...
		p.InputControl.Run(func(c *pipeline.AsyncControl) {
			...
			if err := service.StartService(p.InputPipeContext); err != nil {
				logger.Error(p.LogstoreConfig.Context.GetRuntimeContext(), util.CATEGORY_CONFIG_ALARM, "start service error, err", err)
			}
...
func (p *pluginv2Runner) runMetricInput(control *pipeline.AsyncControl) {
	for _, t := range p.TimerRunner {
		if plugin, ok := t.state.(pipeline.MetricInputV2); ok {
			...
			logger.Error(p.LogstoreConfig.Context.GetRuntimeContext(), util.PLUGIN_START_ALARM, "type assertion", "failure")
		}
	}
}
			...
			for _, aggregator := range p.AggregatorPlugins {
				for _, pipeEvent := range pipeEvents {
					...
					for tryCount := 1; true; tryCount++ {
						...
						if tryCount%100 == 0 {
							logger.Warning(p.LogstoreConfig.Context.GetRuntimeContext(), util.CATEGORY_CONFIG_ALARM, "error", err)
						}
			...
			for {
				...
				if allReady {
					for _, flusher := range p.FlusherPlugins {
						...
						if err != nil {
							logger.Error(p.LogstoreConfig.Context.GetRuntimeContext(), util.CATEGORY_CONFIG_ALARM, "flush data error",
...
func (p *pluginv2Runner) Stop(exit bool) error {
	...
	for idx, flusher := range p.FlusherPlugins {
		if err := flusher.Flusher.Stop(); err != nil {
			logger.Warningf(p.LogstoreConfig.Context.GetRuntimeContext(), util.PLUGIN_STOP_ALARM,
...
func (p *pluginv2Runner) ReceiveLogGroup(in pipeline.LogGroupWithContext) {
	...
	if in.Context != nil {
		for k, v := range in.Context {
			value, ok := v.(string)
			if !ok {
				logger.Warningf(p.LogstoreConfig.Context.GetRuntimeContext(), util.CATEGORY_CONFIG_ALARM, "unknown values found in context, type is %T", v)
				continue
			}
			meta.Add(k, value)
		}
