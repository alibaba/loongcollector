...
func getListenPortsFromFile(pid int, protocol string) ([]int, error) {
	...
	for _, line := range lines[1:] {
		...
		*/
		...
		if err != nil {
			logger.Error(context.Background(), selfmonitor.ReadProcAlarm, err.Error())
			continue
		}
...
func getLogtailLitsenPorts() ([]int, []int) {
	...
	if err != nil {
		logger.Error(context.Background(), selfmonitor.ReadPtoctcpAlarm, err.Error())
	}
	...
	if err != nil {
		logger.Error(context.Background(), selfmonitor.ReadPtoctcp6Alarm, err.Error())
	}
...
func FindPort(res http.ResponseWriter, req *http.Request) {
	...
	if err != nil {
		...
		if WriteErr != nil {
			logger.Error(context.Background(), selfmonitor.HTTPServerError, WriteErr.Error())
		}
		return
	}
