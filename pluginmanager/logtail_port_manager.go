...
func getListenPortsFromFile(pid int, protocol string) ([]int, error) {
	...
	for _, line := range lines[1:] {
		...
		*/
		...
		if err != nil {
			logger.Error(context.Background(), util.INTERNAL_SERVICE_ERROR, "parse port fail", err.Error())
			continue
		}
...
func getLogtailLitsenPorts() ([]int, []int) {
	...
	if err != nil {
		logger.Error(context.Background(), util.INTERNAL_SERVICE_ERROR, "get tcp port fail", err.Error())
	}
	...
	if err != nil {
		logger.Error(context.Background(), util.INTERNAL_SERVICE_ERROR, "get tcp6 port fail", err.Error())
	}
	...
	if err != nil {
		logger.Error(context.Background(), util.INTERNAL_SERVICE_ERROR, "get udp port fail", err.Error())
	}
	...
	if err != nil {
		logger.Error(context.Background(), util.INTERNAL_SERVICE_ERROR, "get udp6 port fail", err.Error())
	}
...
func FindPort(res http.ResponseWriter, req *http.Request) {
	...
	if err != nil {
		...
		if WriteErr != nil {
			logger.Error(context.Background(), util.INTERNAL_SERVICE_ERROR, "write response err", WriteErr.Error())
		}
		return
	}
