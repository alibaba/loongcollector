...
func CollectContainers(logGroup *protocol.LogGroup) {
	if isCollectContainers() {
		if time.Since(lastFetchAllTime) >= FetchAllInterval {
			...
			// timer config result
			dockercenter.RecordContainerConfigResult()
...
func CollectConfigResult(logGroup *protocol.LogGroup) {
	dockercenter.SerializeContainerConfigResultToPb(logGroup)
}
...
func collectAllContainers(logGroup *protocol.LogGroup) {
	...
	dockercenter.SerializeContainerToPb(logGroup, containerDetailToRecords)
...
func collectDiffContainers(logGroup *protocol.LogGroup) {
	...
	fullAddedList, fullDeletedList := dockercenter.GetDiffContainers(cachedFullList)
	...
	if len(fullDeletedList) > 0 {
		...
		for _, containerID := range fullDeletedList {
			if len(containerID) > 0 {
				shortDeletedIDs[dockercenter.GetShortID(containerID)] = struct{}{}
			}
		}
	...
	if len(fullAddedList) > 0 {
		...
		if len(containerIDs) > 0 {
			...
			if len(containers) > 0 {
				dockercenter.SerializeContainerToPb(logGroup, containers)
			}
		}
	}
	{
...
func recordDeleteContainers(logGroup *protocol.LogGroup, containerIDs map[string]struct{}) {
	if len(containerIDs) > 0 {
		...
		projectStr := dockercenter.GetStringFromList(keys)
		...
		dockercenter.SerializeDeleteContainerToPb(logGroup, projectStr, dockercenter.GetStringFromList(ids))
	}
}
...
func getContainersToRecord(containerIDs map[string]struct{}) (map[string]struct{}, []*dockercenter.ContainerDetail) {
	projectSet := make(map[string]struct{})
	...
	projectStr := dockercenter.GetStringFromList(keys)
	// get add container
	result := dockercenter.GetAllContainerToRecord(envSet, containerLabelSet, k8sLabelSet, containerIDs)

	containerDetailToRecords := make([]*dockercenter.ContainerDetail, 0)
	for _, containerInfo := range result {
		var containerDetailToRecord dockercenter.ContainerDetail
...
func compareEnvAndLabelAndRecordContainer() []*dockercenter.ContainerDetail {
	...
	if len(diffEnvSet) != 0 || len(diffContainerLabelSet) != 0 || len(diffK8sLabelSet) != 0 {
		...
		projectStr := dockercenter.GetStringFromList(keys)
		result := dockercenter.GetAllContainerIncludeEnvAndLabelToRecord(envSet, containerLabelSet, k8sLabelSet, diffEnvSet, diffContainerLabelSet, diffK8sLabelSet)
