...
func (p *checkPointManager) SaveCheckpoint(configName, key string, value []byte) error {
	...
	if err != nil {
		logger.Error(context.Background(), util.CheckpointSaveAlarm, "save checkpoint error, key", key, "error", err)
	}
	return err
}
...
func (p *checkPointManager) GetCheckpoint(configName, key string) ([]byte, error) {
	...
	if err != nil && err != leveldb.ErrNotFound {
		logger.Error(context.Background(), util.CheckpointGetAlarm, "get checkpoint error, key", key, "error", err)
	}
	return val, err
}
