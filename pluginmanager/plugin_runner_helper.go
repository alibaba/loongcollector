...
func (p *timerRunner) execTask(task func(state interface{}) error) {
	...
	if err := task(p.state); err != nil {
		logger.Error(p.context.GetRuntimeContext(), util.PLUGIN_RUNTIME_ALARM, "task run", "error", err, "plugin", "state", fmt.Sprintf("%T", p.state))
	}
	if p.latencyMetric != nil {
	...
	for _, flusher := range flushers {
		for waitCount := 0; !flusher.IsReady(lc.ProjectName, lc.LogstoreName, lc.LogstoreKey); waitCount++ {
			if waitCount > maxFlushOutTime*100 {
				logger.Error(lc.Context.GetRuntimeContext(), util.DROP_LOG_ALARM, "flush out data timeout, drop data", store.Len())
				return false
			}
		...
		if err != nil {
			logger.Error(lc.Context.GetRuntimeContext(), util.CATEGORY_CONFIG_ALARM, "flush data error", lc.ProjectName, lc.LogstoreName, err)
		}
