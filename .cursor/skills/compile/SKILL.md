...
1. 判断是否进行增量编译。如果已有 core/build 目录，并且其中有内容，并且你的修改没有涉及到 CMake 相关的文件，那么跳转到第5步进行增量编译。
...
cmake -DCMAKE_BUILD_TYPE=Debug -DLOGTAIL_VERSION=0.0.1 \
      ...
      -DBUILD_LOGTAIL=ON -DBUILD_LOGTAIL_UT=ON -DWITHOUTGDB==ON -DENABLE_STATIC_LINK_CRT=ON -DWITHSPL=ON ..
```

注意其中的几个开关：
    - BUILD_LOGTAIL：表示编译 LoongCollector 二进制。必选
    - BUILD_LOGTAIL_UT：表示编译 LoongCollector 单测。仅当你修改了 LoongCollector 单测时才打开。
    - WITHSPL：表示编译 LoongCollector SPL 相关内容。仅当你修改了 LoongCollector SPL相关文件是才打开。
