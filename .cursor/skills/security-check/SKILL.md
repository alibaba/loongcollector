---
name: security-check
description: Wheneven commit or push is mentioned
---
# 安全检查规则
...
### 先进行检查
使用  `bash scripts/security_check.sh push` 检查每个 commit 中是否包含敏感信息。如果没有输出 `all commits are clear` 那说明存在敏感信息，包含敏感信息的commit hash已被写入 `task/sensitive_commits.txt`。
