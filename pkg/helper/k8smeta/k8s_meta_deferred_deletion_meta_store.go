		...
		for realKey := range realKeys.Keys {
			if obj, ok := m.Items[realKey]; ok {
				if obj.Raw != nil {
					...
					logger.Warning(context.Background(), "K8S_META_HANDLE_ALARM", "raw object not found", realKey)
				}
			} else {
				logger.Warning(context.Background(), "K8S_META_HANDLE_ALARM", "key not found", realKey)
			}
		}
	}
	return result
...
func (m *DeferredDeletionMetaStore) handleEvent() {
	...
	for {
		...
		case event := <-m.eventCh:
			...
			default:
				logger.Warning(context.Background(), "unknown event type", event.EventType)
			}
		case <-m.stopCh:
			m.registerLock.Lock()
...
func (m *DeferredDeletionMetaStore) handleAddOrUpdateEvent(event *K8sMetaEvent) {
	...
	if err != nil {
		logger.Warning(context.Background(), "K8S_META_HANDLE_ALARM", "handle k8s meta with keyFunc error", err)
		return
	}
...
func (m *DeferredDeletionMetaStore) handleDeferredDeleteEvent(event *K8sMetaEvent) {
	key, err := m.keyFunc(event.Object.Raw)
	if err != nil {
		logger.Warning(context.Background(), "handleDeferredDeleteEvent keyFunc error", err)
		return
	}
	idxKeys := m.getIdxKeys(event.Object)
	m.lock.Lock()
...
func (m *DeferredDeletionMetaStore) getIdxKeys(obj *ObjectWrapper) []string {
	...
	for _, rule := range m.indexRules {
		...
		if err != nil {
			logger.Warning(context.Background(), "K8S_META_HANDLE_ALARM", "handle k8s meta with idx rules error", err)
			return nil
		}
