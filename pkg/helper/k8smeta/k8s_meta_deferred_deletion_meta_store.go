		...
		for realKey := range realKeys.Keys {
			if obj, ok := m.Items[realKey]; ok {
				if obj.Raw != nil {
					...
					logger.Error(context.Background(), util.K8sMetaAlarm, "handle k8s meta with raw object not found error", realKey)
				}
			} else {
				logger.Error(context.Background(), util.K8sMetaAlarm, "handle k8s meta with key not found error", realKey)
			}
		}
	}
	return result
...
func (m *DeferredDeletionMetaStore) handleEvent() {
	...
	for {
		...
		case event := <-m.eventCh:
			...
			default:
				logger.Error(context.Background(), util.K8sMetaAlarm, "handle k8s meta with unknown event type error", event.EventType)
			}
		case <-m.stopCh:
			m.registerLock.Lock()
...
func (m *DeferredDeletionMetaStore) handleAddOrUpdateEvent(event *K8sMetaEvent) {
	...
	if err != nil {
		logger.Error(context.Background(), util.K8sMetaAlarm, "handle k8s meta with keyFunc error", err)
		return
	}
...
func (m *DeferredDeletionMetaStore) handleDeferredDeleteEvent(event *K8sMetaEvent) {
	key, err := m.keyFunc(event.Object.Raw)
	if err != nil {
		logger.Error(context.Background(), util.K8sMetaAlarm, "handle k8s meta with DeferredDeleteEvent keyFunc error", err)
		return
	}
	idxKeys := m.getIdxKeys(event.Object)
	m.lock.Lock()
...
func (m *DeferredDeletionMetaStore) getIdxKeys(obj *ObjectWrapper) []string {
	...
	for _, rule := range m.indexRules {
		...
		if err != nil {
			logger.Error(context.Background(), util.K8sMetaAlarm, "handle k8s meta with idx rules error", err)
			return nil
		}
