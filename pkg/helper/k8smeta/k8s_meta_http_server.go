...
func (m *metadataHandler) handlePodMetaByContainerID(w http.ResponseWriter, r *http.Request) {
	...
	for key, obj := range objs {
		...
		if len(podMetadata) > 1 {
			logger.Warning(context.Background(), util.K8sMetaAlarm, "Multiple pods found for unique container ID", key)
		}
...
func (m *metadataHandler) getCommonPodMetadata(pod *v1.Pod) *PodMetadata {
	...
	if len(pod.GetOwnerReferences()) == 0 {
		...
		logger.Warning(context.Background(), util.K8sMetaAlarm, "Pod has no owner", pod.Name)
	} else {
		...
		if podMetadata.WorkloadKind == REPLICASET {
			...
			if podMetadata.WorkloadKind == "replicaset" {
				logger.Warning(context.Background(), util.K8sMetaAlarm, "ReplicaSet has no owner", podMetadata.WorkloadName)
			}
		}
	}
	return podMetadata
