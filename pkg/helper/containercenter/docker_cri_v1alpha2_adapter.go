...
func V1Alpha2StateToInt32(state criv1alpha2.ContainerState) int32 {
	return int32(state)
}

// createContainerInfo convert cri container to docker spec to adapt the history logic.
func (cw *CRIV1Alpha2Wrapper) createContainerInfo(containerID string) (detail *DockerInfoDetail, sandboxID string, state int32, err error) {
	...
	cancel()
	if err != nil {
		return nil, "", V1Alpha2StateToInt32(criv1alpha2.ContainerState_CONTAINER_UNKNOWN), err
	}
	...
	if !foundInfo {
		...
		return nil, "", V1Alpha2StateToInt32(criv1alpha2.ContainerState_CONTAINER_UNKNOWN), fmt.Errorf("can not find container info from CRI::ContainerStatus, containerId: %s", containerID)
	}
	...
	state = V1Alpha2StateToInt32(status.Status.State)
	stateStatus := ContainerStatusExited
	if state == V1Alpha2StateToInt32(criv1alpha2.ContainerState_CONTAINER_RUNNING) && ContainerProcessAlive(int(ci.Pid)) {
...
func (cw *CRIV1Alpha2Wrapper) fetchAll() error {
	...
	for i, c := range containersResp.Containers {
		...
		cw.containers[c.GetId()] = &innerContainerInfo{
			State:  V1Alpha2StateToInt32(c.State),
...
func (cw *CRIV1Alpha2Wrapper) syncContainers() error {
	...
	for i, c := range containersResp.Containers {
		...
		if ok {
			...
			if oldInfo.State != V1Alpha2StateToInt32(criv1alpha2.ContainerState_CONTAINER_RUNNING) || // not running
				(oldInfo.State == V1Alpha2StateToInt32(c.State) && oldInfo.Name == c.Metadata.Name && oldInfo.Status == status) { // no state change
				continue
			}
		} else if inHistory {
			continue
	...
	for oldID, c := range cw.containers {
		if _, ok := newContainers[oldID]; !ok || c.State == V1Alpha2StateToInt32(criv1alpha2.ContainerState_CONTAINER_EXITED) {
