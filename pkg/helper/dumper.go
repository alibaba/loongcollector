...
func (d *Dumper) Init() {
	...
	if err != nil {
		logger.Warning(context.Background(), selfmonitor.ListHistoryDumpAlarm, "err", err)
	} else {
	...
	for {
		...
		case data := <-d.input:
			if time.Now().Hour() != lastHour {
				file, cerr := cutFile()
				if cerr != nil {
					logger.Warning(context.Background(), selfmonitor.DumpFileAlarm, "cut new file error", err)
				} else {
