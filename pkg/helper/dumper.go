...
func (d *Dumper) Init() {
	...
	if err != nil {
		logger.Warning(context.Background(), util.DumpDataAlarm, "list history dump err", err)
	} else {
	...
	for {
		...
		case data := <-d.input:
			if time.Now().Hour() != lastHour {
				file, cerr := cutFile()
				if cerr != nil {
					logger.Error(context.Background(), util.DumpDataAlarm, "cut new file error", err)
				} else {
