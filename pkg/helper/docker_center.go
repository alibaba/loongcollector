	...
	onceDocker.Do(func() {
		...
		go func() {
			...
			for {
				...
				if retryCount%10 == 0 {
					logger.Error(context.Background(), util.DOCKER_CENTER_ALARM, "docker center init failed", "retry count", retryCount)
				}
				retryCount++
...
func (dc *DockerCenter) readStaticConfig(forceFlush bool) {
	...
	if err != nil {
		logger.Warning(context.Background(), util.DOCKER_CENTER_ALARM, "read static container info error", err)
	}
...
func (dc *DockerCenter) setLastError(err error, msg string) {
	...
	dc.lastErrMu.Unlock()
	if err != nil {
		logger.Warning(context.Background(), util.DOCKER_CENTER_ALARM, "message", msg, "error found", err)
	} else {
...
func (dc *DockerCenter) getAllAcceptedInfoV2(
	...
	for id := range matchList {
		...
		if ok {
			...
			logger.Warningf(context.Background(), util.DOCKER_CENTER_ALARM, "matched container not in docker center")
		}
	}
...
func dockerCenterRecover() {
	if err := recover(); err != nil {
		...
		logger.Error(context.Background(), util.PLUGIN_RUNTIME_ALARM, "docker center runtime error", err, "stack", string(trace))
	}
}
	...
	for {
		...
		for !breakFlag {
			...
			case event, ok := <-events:
				if !ok {
					logger.Errorf(context.Background(), util.DOCKER_CENTER_ALARM, "docker event listener stop")
					errorCount++
					breakFlag = true
					break
				}
				...
				if dc.eventChan != nil {
					...
					default:
						logger.Error(context.Background(), util.DOCKER_CENTER_ALARM, "event queue is full, miss event", event)
					}
				}
				dc.eventChanLock.Unlock()
			case err = <-errors:
				logger.Error(context.Background(), util.DOCKER_CENTER_ALARM, "docker event listener error", err)
				breakFlag = true
			case <-timer.C:
				logger.Errorf(context.Background(), util.DOCKER_CENTER_ALARM, "no docker event in 1 hour. Reset event listener")
				breakFlag = true
			}
		}
		cancel()
