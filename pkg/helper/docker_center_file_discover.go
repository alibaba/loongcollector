...
func scanContainerdFilesAndReLink(filePath string) {
	...
	if err := overWriteSymLink(filepath.Join(dirPath, strconv.Itoa(maxFileNo)+logSuffix), filePath); err != nil {
		logger.Error(context.Background(), util.DOCKER_CENTER_ALARM, "overwrite symbol link error, from", maxFileNo, "to", filePath, "error", err)
	} else {
	...
	for {
		...
		for _, fi := range dir {
			...
			if err := overWriteSymLink(filepath.Join(dirPath, strconv.Itoa(fileNo)+logSuffix), filePath); err == nil {
				...
				logger.Error(context.Background(), util.DOCKER_CENTER_ALARM, "overwrite symbol link error, from", fileNo, "to", filePath, "error", err, "max", maxFileNo)
			}
		}
	}
}
