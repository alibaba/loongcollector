...
func (c *ContainerDiscoverManager) Init() bool {
	...
	if IsCRIRuntimeValid(containerdUnixSocket) {
		...
		if err != nil {
			logger.Errorf(context.Background(), util.DOCKER_CENTER_ALARM, "[CRIRuntime] creare cri-runtime client error: %v", err)
	...
	if ok, err := util.PathExists(DefaultLogtailMountPath); err == nil {
		...
		logger.Warning(context.Background(), util.DOCKER_CENTER_ALARM, "check docker mount path error", err.Error())
	}
	...
	if c.enableDockerDiscover {
		if err = c.fetchDocker(); err != nil {
			...
			logger.Errorf(context.Background(), util.DOCKER_CENTER_ALARM, "fetch docker containers error, close docker discover, will retry")
		}
	}
	if c.enableCRIDiscover {
		if err = c.fetchCRI(); err != nil {
			c.enableCRIDiscover = false
			logger.Errorf(context.Background(), util.DOCKER_CENTER_ALARM, "fetch cri containers error, close cri discover, will retry")
		}
	}
	if c.enableStaticDiscover {
		c.fetchStatic()
