...
func (d *Decoder) parseInputMeta(req *http.Request) (*profile.Input, profile.Format, error) {
	...
	if err != nil {
		logger.Error(context.Background(), util.PARSE_LOG_FAIL_ALARM, "invalid name", n)
	...
	if sr := q.Get("sampleRate"); sr != "" {
		...
		if err != nil {
			logger.Error(context.Background(), util.PARSE_LOG_FAIL_ALARM, "invalid sampleRate", sr)
		} else {
