...
// res:
// if the next metric has the same labels with the previous ones. Same Tags means:
// 1. count is the same.
// 2. tag keys and values are the same.
func hasDifferentLabels(currLabels map[string]string, commonLabels map[string]string, nextMeasurement map[string]string) bool {
	...
	// Note that currLabels is already bound to more than 1 metrics, if it is empty, it means the previous metrics have no tags

	nextMeasurementLabelCount := 0
	for key, value := range nextMeasurement {
		...
		// If label does not exist or values differs, labels are different
		currValue, exists := currLabels[key]
		if !exists || currValue != value {
			return true
		}
		nextMeasurementLabelCount++
	}

	// Check if common labels differ from previous labels, which only happens if the previous metrics override the common labels.
	// note the previous labels must contains the common label keys.
	for key, value := range commonLabels {
		if value != currLabels[key] {
			return true
		}

		// if nextMeasurement override the common labels, labelCount will not incr.
		if _, ok := nextMeasurement[key]; !ok {
			nextMeasurementLabelCount++
		}
	}

	// here all values in nextMeasurement are already contained in currLabels.
	// if nextMeasurementLabelCount != len(currLabels), it means nextMeasurement has more labels than currLabels.
	return nextMeasurementLabelCount != len(currLabels)
}
