...
# in the Docker build (docker/Dockerfile_build uses loongcollector-build-linux 2.1.19+ for packages).
CATEGORY=$1
GENERATED_HOME=$2
VERSION=${3:-0.0.1}
...
EOF
  ...
  # Perl/OpenSSL build deps come from loongcollector-build-linux 2.1.19+; only verify here.
...
perl -MIPC::Cmd -e1 && perl -MData::Dumper -e1 && perl -MTime::Piece -e1 || { echo "ERROR: perl IPC::Cmd, Data::Dumper or Time::Piece missing (use loongcollector-build-linux 2.1.19+ or install AgentSight build deps)." >&2; exit 1; }
