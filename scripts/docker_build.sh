...
#
# AgentSight (coolbpf vendored openssl-sys): perl + openssl-devel come from loongcollector-build-linux 2.1.18+.
...
if [[ "$CATEGORY" = "build" || "$CATEGORY" = "development" ]]; then
    if ! grep -qE 'loongcollector-build-linux:2\.1\.(1[89]|[2-9][0-9])|loongcollector-build-linux:2\.[2-9]|loongcollector-build-linux:[3-9]' "$GEN_DOCKERFILE"; then
        echo "WARNING: AgentSight expects loongcollector-build-linux 2.1.18+ in Dockerfile_build FROM; check generated Dockerfile." >&2
    fi
fi
