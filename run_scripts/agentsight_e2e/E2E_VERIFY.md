...
| `run_scripts/agentsight_e2e/verify_baseline_delta.sh` | **baseline 专用**：SLS 校验 delta 公式；支持 `AGENT_TYPE` |
| `run_scripts/agentsight_e2e/run_baseline_e2e.sh` | 一键 baseline：可选 `--deploy` + trigger + delta 校验；`AGENT_TYPE=hermes` |
...
**Session ID 说明**：OpenClaw CLI 的 `--session-id` 与 Hermes CLI 打印的 `session_id:` **不一定**等于 SLS 里的 `gen_ai.session.id`。推荐用 **marker 检索**（Hermes 自动嵌入 `E2E_HERMES_*`；OpenClaw baseline 消息含 `BASELINE_OK`）。`verify_delta_baseline.py` 在提供 `--marker` 时会自动从 SLS 解析真实 `gen_ai.session.id`（`--resolve-session`，默认开启）。
...
- [ ] **`baseline` delta 公式**（自动或手动 `verify_baseline_delta.sh`）：见 §7.1
...
### 7.1 Baseline-only delta 公式校验

**仅 `baseline` case**（`EventStreamFormat=false` / `MessageDeltaOnly=false`）执行严格 delta 公式校验。其余 case 只做字段形态检查，**不**验证该公式。

同一 `gen_ai.session.id` 内按 `__time__` 升序排列、去重后的每条 HTTP 合并日志，应满足：
...
| 符号 | 含义 |
|------|------|
| `canon` | 每条 message 仅保留 `role` + `parts` 后 JSON 比较 |
| `omit_system` | 从 messages 数组去掉 `role=system` |
| `prev_in` | 上一条 HTTP 的 `gen_ai.input.messages` |
| `prev_out` | 上一条 HTTP 的 `gen_ai.output.messages` |
| `delta` | 当前条的 `gen_ai.input.messages.delta` |
| `cur` | 当前条的 `gen_ai.input.messages` |

**首条 HTTP**（session 内无 prior）：`delta` 应等于 `omit_system(cur)`。

**场景标签**（脚本输出 `[scenario]`）：

| 标签 | 含义 |
|------|------|
| `first_in_session` | session 内第一条 HTTP |
| `same_turn_step_2` | 同一 `gen_ai.turn.id` 内第 2+ 次 HTTP（agent 多步 tool loop） |
| `cross_turn_step_1` | 新 turn 的首条 HTTP（跨轮用户消息） |

**SLS 去重**：同一 `gen_ai.response.id` 可能出现重复条（pipeline flush 镜像）；脚本按 `response.id` 去重，签名比较时忽略 `gen_ai.step.id`（baseline 无此字段）。

**其他 case 校验范围**（形态 only，无 delta 公式）：

| Case | 校验内容 |
|------|----------|
| `split` | 成对 request/response、`event.id` 不同、字段分工 |
| `no-detail` | 无 full input/system/tools；有 delta + output |
| `split-no-detail` | request 仅 delta/hash；response 有 output |
...
# OpenClaw：已知 session-id（trigger 脚本会打印；可能与 SLS 不同）
bash run_scripts/agentsight_e2e/verify_baseline_delta.sh \
  --session-id e2e-baseline-1717654321 --wait 60

# OpenClaw：marker 检索（工具 E2E 或含 BASELINE_OK 的 baseline 消息）
bash run_scripts/agentsight_e2e/verify_baseline_delta.sh \
  --marker TOOL_OK --minutes 20

# Hermes：marker 检索（推荐；自动解析 SLS gen_ai.session.id）
bash run_scripts/agentsight_e2e/verify_baseline_delta.sh \
  --agent-type hermes --marker E2E_HERMES_20260606_110133 --wait 60

# 一键：trigger + 自动校验（需已 deploy baseline）
bash run_scripts/agentsight_e2e/run_baseline_e2e.sh

# Hermes 一键
AGENT_TYPE=hermes bash run_scripts/agentsight_e2e/run_baseline_e2e.sh

# 含 deploy + 工具两轮（仅 openclaw）
...
```text
AgentSight baseline delta verify — session=e2e-tools-1717654321
Config: .../loongcollector_config.json | window: ... | fetched=6 deduped=3
  PASS [first_in_session] turn=turn-abc response.id=resp-001
  PASS [same_turn_step_2] turn=turn-abc response.id=resp-002
  PASS [cross_turn_step_1] turn=turn-def response.id=resp-003
