...
uint64_t TraceIDToRandomness(const std::array<uint8_t, 16>&traceID) {
    uint64_t half = (static_cast<uint64_t>(traceID[15])) |
                    (static_cast<uint64_t>(traceID[14]) << 8) |
                    (static_cast<uint64_t>(traceID[13]) << 16) |
                    (static_cast<uint64_t>(traceID[12]) << 24) |
                    (static_cast<uint64_t>(traceID[11]) << 32) |
                    (static_cast<uint64_t>(traceID[10]) << 40) |
                    (static_cast<uint64_t>(traceID[9]) << 48) |
                    (static_cast<uint64_t>(traceID[8]) << 56);
...
RatioSampler::RatioSampler(const double fraction, const uint64_t thresHold): fraction_(fraction),
...
HashRatioSampler::HashRatioSampler(const double fraction): RatioSampler(
    fraction, ProbabilityToThreshold(fraction)) {
}
