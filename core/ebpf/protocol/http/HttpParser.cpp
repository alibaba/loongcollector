...
// //            "  response_msg: " << data_event->resp_msg.data() << " tgid:" << data_event->conn_id.tgid << " fd:" << data_event->conn_id.fd << " start:" << data_event->conn_id.start;
...
// //   LOG(INFO) << "HTTP Parser DUMP conn_id ["  << conn_id.tgid << " fd:" << conn_id.fd << " start:" << conn_id.start;
...
//     LOG_IF(WARNING, state != ParseState::kSuccess) << "[HTTPProtocolParser]: Parse HTTP request failed: "<<(size_t) state;
...
//     LOG_IF(WARNING, state != ParseState::kSuccess) << "[HTTPProtocolParser]: Parse HTTP response failed: "<<(size_t) state;
...
//         auto r = ParseContent(content_len_str, buf, 256, &result->body,
//                             &result->body_size);
//         return r;
//     }
...
//     if (content_length_iter != result->headers.end()) {
//         std::string_view content_len_str = content_length_iter->second;
//         auto s = ParseContent(content_len_str, buf, 256, &result->body,
//                             &result->body_size);
