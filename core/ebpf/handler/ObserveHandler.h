...
public:
    MeterHandler(const logtail::CollectionPipelineContext* ctx, QueueKey key, uint32_t idx) : AbstractHandler(ctx, key, idx) {}
...
public:
    OtelMeterHandler(const logtail::CollectionPipelineContext* ctx, QueueKey key, uint32_t idx) : MeterHandler(ctx, key, idx) {}
...
public:
    SpanHandler(const logtail::CollectionPipelineContext* ctx, QueueKey key, uint32_t idx) : AbstractHandler(ctx, key, idx) {}
    virtual void handle(const std::vector<std::unique_ptr<ApplicationBatchSpan>>&) = 0;
};

class OtelSpanHandler : public SpanHandler {
public:
    OtelSpanHandler(const logtail::CollectionPipelineContext* ctx, QueueKey key, uint32_t idx) : SpanHandler(ctx, key, idx) {}
    void handle(const std::vector<std::unique_ptr<ApplicationBatchSpan>>&) override;
};

class EventHandler : public AbstractHandler {
public:
    EventHandler(const logtail::CollectionPipelineContext* ctx, QueueKey key, uint32_t idx) : AbstractHandler(ctx, key, idx) {}
