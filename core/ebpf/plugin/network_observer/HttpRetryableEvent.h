...
public:
    HttpRetryableEvent(int retryLimit, const std::shared_ptr<HttpRecordV2>& record, moodycamel::BlockingConcurrentQueue<std::shared_ptr<CommonEvent>>& queue)
...
private:
    ...
    // record ...
    std::shared_ptr<HttpRecordV2> mRecord;
