...
GetAppDetail(const std::unordered_map<size_t, std::shared_ptr<AppDetail>>& currentContainerConfigs, size_t key) {
    ...
    const auto it2 = currentContainerConfigs.find(0);
...
NetworkObserverManager::NetworkObserverManager(const std::shared_ptr<ProcessCacheManager>& processCacheManager,
    ...
    : AbstractManager(processCacheManager, eBPFAdapter, threadPool, queue, nullptr),
      mAppAggregator(
          10240,
      ...
      mNetAggregator(
          ...
          [this](const std::shared_ptr<AbstractRecord>& i,
                 std::shared_ptr<SourceBuffer>& sourceBuffer) -> std::unique_ptr<NetMetricData> {
      ...
      mConnectionNum(std::move(connNum)),
      mPollNetEventsTotal(std::move(pollEvents)),
      mLossNetEventsTotal(std::move(lossEvents)),
      mNetMetaAttachSuccessTotal(std::move(attachSuccess)),
      mNetMetaAttachFailedTotal(std::move(attachFailed)),
      mNetMetaAttachRollbackTotal(std::move(rollbackTotal)) {
}
...
int NetworkObserverManager::Init(const std::variant<SecurityOptions*, ObserverNetworkOption*>&) {
    ...
    mConnectionManager->RegisterConnStatsFunc(
        [this](const std::unordered_map<size_t, std::shared_ptr<AppDetail>>& currentContainerConfigs,
               std::shared_ptr<AbstractRecord>& record) { processRecord(currentContainerConfigs, record); });
...
bool NetworkObserverManager::ScheduleNext(const std::chrono::steady_clock::time_point& execTime,
    ...
    mThreadPool->Add([this, noConfig, execTime]() { this->executeTimerTask(noConfig->mJobType, execTime); });
...
void NetworkObserverManager::processRecordAsLog(const std::shared_ptr<AbstractRecord>& record,
                                                const std::shared_ptr<logtail::ebpf::AppDetail>& appInfo) {
    WriteLock lk(mLogAggLock);
    auto res = mLogAggregator.Aggregate(record, generateAggKeyForLog(record, appInfo));
...
void NetworkObserverManager::processRecordAsSpan(const std::shared_ptr<AbstractRecord>& record,
                                                 const std::shared_ptr<logtail::ebpf::AppDetail>& appInfo) {
    WriteLock lk(mSpanAggLock);
    auto res = mSpanAggregator.Aggregate(record, generateAggKeyForSpan(record, appInfo));
...
void NetworkObserverManager::processRecord(
    ...
    switch (record->GetRecordType()) {
        case RecordType::APP_RECORD: {
            ...
            auto conn = appRecord->GetConnection();
            if (!appRecord || !appRecord->GetConnection()) {
                // should not happen
                return;
            }

            if (!conn->IsMetaAttachReadyForAppRecord() && conn->IsConnDeleted()) {
                // try attach again, for sake of connection is released in connection manager ...
                conn->TryAttachPeerMeta();
                conn->TryAttachSelfMeta();
            }

            if (!conn->IsMetaAttachReadyForAppRecord()) {
                // rollback
            ...
            const auto& appInfo = GetAppDetail(currentContainerConfigs, conn->GetContainerIdKey());
            if (appInfo == nullptr) {
                return;
            }

            // handle record
            if (appInfo->mEnableLog && record->ShouldSample()) {
                processRecordAsLog(record, appInfo);
            }
            if (appInfo->mEnableMetric) {
                // TODO(qianlu): add converge ...
                // aggregate ...
                processRecordAsMetric(record, appInfo);
            }
            if (appInfo->mEnableSpan && record->ShouldSample()) {
                processRecordAsSpan(record, appInfo);
            }
            break;
        }
        case RecordType::CONN_STATS_RECORD: {
            ...
            ADD_COUNTER(mNetMetaAttachSuccessTotal, 1);
            const auto& appInfo
                = GetAppDetail(currentContainerConfigs, connStatsRecord->GetConnection()->GetContainerIdKey());
            if (appInfo == nullptr) {
                return;
            }
            ...
            // do aggregate
            {
                ...
                auto res = mNetAggregator.Aggregate(record, generateAggKeyForNetMetric(record, appInfo));
...
void NetworkObserverManager::ConsumeRecords() {
    ...
    std::unordered_map<size_t, std::shared_ptr<AppDetail>> currentConfigs;
    ...
    while (mFlag) {
        ...
        // copy ...
        {
            ...
            if (currentVersion != mConfigVersion) {
                currentConfigs = mContainerConfigs;
            }
        }
        for (size_t i = 0; i < count; i++) {
            processRecord(currentConfigs, items[i]);
        }
...
void NetworkObserverManager::PollBufferWrapper() {
    ...
    while (this->mFlag) {
        ...
        // TODO(qianlu.kk): We need to dump a duplication for container configs
        // so that we can do lock-free operations ...
        {
            ReadLock lk(mAppConfigLock);
            if (currentVersion != mConfigVersion) {
        ...
        mConnectionManager->Iterations(mContainerConfigsReplica);
...
void NetworkObserverManager::AcceptDataEvent(struct conn_data_event_t* event) {
    ...
    const auto& appInfo = getAppConfigFromReplica(conn);
