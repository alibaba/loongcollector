...
public:
    ...
    bool Init(const KafkaConfig& config) {
        ...
        SetConfig(KAFKA_CONFIG_QUEUE_BUFFERING_MAX_KBYTES, std::to_string(mConfig.Producer.QueueBufferingMaxKbytes));
        ...
        SetConfig(KAFKA_CONFIG_BATCH_NUM_MESSAGES, std::to_string(mConfig.Producer.BatchNumMessages));
        ...
        SetConfig(KAFKA_CONFIG_ACKS, mConfig.Delivery.Acks);
...
private:
    bool SetConfig(const std::string& key, const std::string& value) {
        ...
        if (rd_kafka_conf_set(mConf, key.c_str(), value.c_str(), errstr, sizeof(errstr)) != RD_KAFKA_CONF_OK) {
            LOG_ERROR(sLogger, ("Failed to set config", "")("key", key)("value", value)("error", errstr));
            return false;
        }
        return true;
    }
