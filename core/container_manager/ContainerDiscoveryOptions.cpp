...
bool ContainerDiscoveryOptions::Init(const Json::Value& config, const CollectionPipelineContext& ctx, const string& pluginType) {
    string errorMsg;
...
void ContainerDiscoveryOptions::GenerateContainerMetaFetchingGoPipeline(
    ...
    plugin["type"] = Json::Value(CollectionPipeline::GenPluginTypeWithID("metric_container_info", pluginMeta.mPluginID));
