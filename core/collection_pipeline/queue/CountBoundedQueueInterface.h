...
template <typename T>
class CountBoundedQueueInterface : virtual public QueueInterface<T> {
public:
    CountBoundedQueueInterface(QueueKey key, size_t cap, size_t low, size_t high, const CollectionPipelineContext& ctx)
        : QueueInterface<T>(key, cap, ctx), mLowWatermark(low), mHighWatermark(high) {
        this->mMetricsRecordRef.AddLabels({{METRIC_LABEL_KEY_QUEUE_TYPE, "count_bounded"}});
    ...
    virtual ~CountBoundedQueueInterface() = default;

    CountBoundedQueueInterface(const CountBoundedQueueInterface& que) = delete;
    CountBoundedQueueInterface& operator=(const CountBoundedQueueInterface&) = delete;
