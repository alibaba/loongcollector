...
bool LinuxSystemInterface::GetHostSystemStat(vector<string> &lines,
                                             string &errorMessage) {
  errorMessage.clear();
  if (!CheckExistance(PROCESS_DIR / PROCESS_STAT)) {
    errorMessage =
        "file does not exist: " + (PROCESS_DIR / PROCESS_STAT).string();
    return false;
  }

  int ret =
      GetFileLines(PROCESS_DIR / PROCESS_STAT, lines, true, &errorMessage);
...
bool LinuxSystemInterface::GetHostLoadavg(vector<string> &lines,
                                          string &errorMessage) {
  errorMessage.clear();
  if (!CheckExistance(PROCESS_DIR / PROCESS_LOADAVG)) {
    errorMessage =
        "file does not exist: " + (PROCESS_DIR / PROCESS_LOADAVG).string();
    return false;
  }

  int ret =
      GetFileLines(PROCESS_DIR / PROCESS_LOADAVG, lines, true, &errorMessage);
  if (ret != 0 || lines.empty()) {
    return false;
  }
  return true;
}
bool LinuxSystemInterface::ReadSocketStat(const std::filesystem::path &path,
                                          uint64_t &tcp) {
  tcp = 0;
  if (!path.empty()) {
    ...
    if (GetFileLines(path, sockstatLines, true, &errorMessage) != 0 ||
        sockstatLines.empty()) {
      return false;
    }

    for (auto const &line : sockstatLines) {
      if (FastParse::FieldStartsWith(line, 0, "TCP:") ||
          FastParse::FieldStartsWith(line, 0, "TCP6:")) {
        ...
        if (fieldCount >= 7) {
          if (!FastParse::GetFieldAs(line, 6, twValue)) {
            LOG_WARNING(sLogger,
                        ("ReadSocketStat, failed to get tw value", line));
          }
        }
        if (fieldCount >= 9) {
          if (!FastParse::GetFieldAs(line, 8, allocValue)) {
            LOG_WARNING(sLogger,
                        ("ReadSocketStat, failed to get alloc value", line));
          }
        }
...
bool LinuxSystemInterface::ReadProcNetTcp(
    std::vector<uint64_t> &tcpStateCount) {
  // Read /proc/net/tcp and /proc/net/tcp6
  std::vector<std::filesystem::path> tcpFiles = {
      PROCESS_DIR / PROCESS_NET_TCP, PROCESS_DIR / PROCESS_NET_TCP6};

  for (const auto &tcpFile : tcpFiles) {
    if (!CheckExistance(tcpFile)) {
      LOG_DEBUG(sLogger,
                ("TCP state file does not exist, skipping", tcpFile.string()));
      continue;
    }

    std::vector<std::string> tcpLines;
    std::string errorMessage;
    if (GetFileLines(tcpFile, tcpLines, true, &errorMessage) != 0 ||
        tcpLines.empty()) {
      LOG_WARNING(sLogger, ("Failed to read TCP state file",
                            tcpFile.string())("error", errorMessage));
      continue;
    }
...
bool LinuxSystemInterface::ReadNetLink(std::vector<uint64_t> &tcpStateCount) {
  ...
  if (fd < 0) {
    LOG_WARNING(sLogger, ("ReadNetLink, socket(AF_NETLINK, SOCK_RAW, "
  ...
  if (bind(fd, (struct sockaddr *)&nladdr_bind, sizeof(nladdr_bind))) {
    LOG_WARNING(sLogger,
                ("ReadNetLink, bind netlink socket failed, error msg: ",
                 std::string(strerror(errno))));
    close(fd);
    return false;
  }
  ...
  if (sendmsg(fd, &msg, 0) < 0) {
    LOG_WARNING(sLogger, ("ReadNetLink, sendmsg(2) failed, error msg: ",
                          std::string(strerror(errno))));
    close(fd);
    return false;
  }
  ...
  while (received_count < MAX_RECV_COUNT) {
    ...
    if (status < 0) {
      ...
      LOG_WARNING(sLogger, ("ReadNetLink, recvmsg(2) failed, error msg: ",
                            std::string(strerror(errno))));
      close(fd);
      return false;
    } else if (status == 0) {
      LOG_WARNING(sLogger, ("ReadNetLink, Unexpected zero-sized  reply from "
    ...
    for (auto h = (struct nlmsghdr *)buf; NLMSG_OK(h, status);
         h = NLMSG_NEXT(h, status)) {
      if (static_cast<uint64_t>(h->nlmsg_seq) !=
      ...
      if (h->nlmsg_type == NLMSG_DONE) {
        ...
        if (h->nlmsg_len < NLMSG_LENGTH(sizeof(struct nlmsgerr))) {
          ...
          LOG_WARNING(sLogger,
                      ("ReadNetLink, Received error, error msg: ", msg_error));
        }
        close(fd);
        return false;
      }
...
bool LinuxSystemInterface::GetNetStateByNetLink(NetState &netState) {
  ...
  std::call_once(initFlag, [this, &tcpStateCount, &success, &ranCallOnce]() {
    ...
    if (success) {
      ...
      LOG_INFO(sLogger, ("Netlink INET_DIAG is available, will use it for TCP "
                         "state collection",
                         ""));
    } else {
      netlinkAvailable = false;
      LOG_INFO(sLogger, ("Netlink INET_DIAG not available, will use "
  ...
  if (!ranCallOnce) {
    success = netlinkAvailable ? ReadNetLink(tcpStateCount)
                               : ReadProcNetTcp(tcpStateCount);
  }
  ...
  netState.tcpStates[TCP_NON_ESTABLISHED] =
      netState.tcpStates[TCP_TOTAL] - netState.tcpStates[TCP_ESTABLISHED];
  return true;
}

bool LinuxSystemInterface::GetHostNetDev(vector<string> &lines,
                                         string &errorMessage) {
  errorMessage.clear();
  if (!CheckExistance(PROCESS_DIR / PROCESS_NET_DEV)) {
    errorMessage =
        "file does not exist: " + (PROCESS_DIR / PROCESS_NET_DEV).string();
    return false;
  }

  int ret =
      GetFileLines(PROCESS_DIR / PROCESS_NET_DEV, lines, true, &errorMessage);
  if (ret != 0 || lines.empty()) {
    return false;
  }
  return true;
...
bool LinuxSystemInterface::GetInterfaceConfig(InterfaceConfig &interfaceConfig,
  ...
  if ((sock = socket(AF_INET, SOCK_DGRAM, 0)) < 0) {
    LOG_WARNING(sLogger, ("socket(AF_INET, SOCK_DGRAM, 0)",
                          std::string(strerror(errno))));
    return false;
  }
  {
  ...
  if (CheckExistance(PROCESS_DIR / PROCESS_NET_IF_INET6)) {
    int ret = GetFileLines(PROCESS_DIR / PROCESS_NET_IF_INET6, netInet6Lines,
                           true, &errorMessage);
    if (ret != 0 || netInet6Lines.empty()) {
      // Failure should not be returned without "/proc/net/if_inet6"
      close(sock);
      LOG_WARNING(sLogger, ("GetInterfaceConfig, failed to get ipv6 config",
                            errorMessage));
      return true;
    }
  }
...
bool LinuxSystemInterface::GetSystemInformationOnce(
      ...
      if (!StringTo(cpuMetric[1], systemInfo.bootTime)) {
        LOG_WARNING(sLogger,
                    ("failed to get system boot time",
                     "use current time instead")("error msg", cpuMetric[1]));
        return false;
      }
      break;
    }
...
bool LinuxSystemInterface::GetCPUInformationOnce(CPUInformation &cpuInfo) {
  ...
  for (auto const &line : cpuLines) {
    ...
    parser.GetCpuStats(cpuStat.user, cpuStat.nice, cpuStat.system, cpuStat.idle,
...
bool LinuxSystemInterface::GetProcessListInformationOnce(
    ProcessListInformation &processListInfo) {
  processListInfo.pids.clear();
  if (!std::filesystem::exists(PROCESS_DIR) ||
      !std::filesystem::is_directory(PROCESS_DIR)) {
    LOG_ERROR(sLogger, ("process root path is not a directory or not exist",
                        PROCESS_DIR));
    return false;
  }
  ...
  try {
    auto it = std::filesystem::directory_iterator(
        PROCESS_DIR, std::filesystem::directory_options::skip_permission_denied,
        ec);
    while (it != std::filesystem::directory_iterator()) {
      if (ec) {
        LOG_WARNING(sLogger,
                    ("failed to iterate process directory", PROCESS_DIR)(
                        "skipping invalid directory entry", ec.message()));
        ec.clear();       // 重置错误码继续遍历
    ...
    LOG_WARNING(
        sLogger,
        ("unexpected exception during process directory iteration", e.what()));
    return false;
  }
...
bool LinuxSystemInterface::GetProcessInformationOnce(
    pid_t pid, ProcessInformation &processInfo) {
  auto processStat = PROCESS_DIR / std::to_string(pid) / PROCESS_STAT;
  std::string line;
  if (FileReadResult::kOK !=
      ReadFileContent(processStat.string(), line, kDefaultMaxFileSize)) {
...
bool LinuxSystemInterface::GetSystemLoadInformationOnce(
  ...
  if (!GetHostLoadavg(loadLines, errorMessage) || loadLines.empty()) {
    LOG_WARNING(sLogger,
                ("failed to get system load",
                 "invalid System collector")("error msg", errorMessage));
    return false;
  }
  ...
  if (!SystemInterface::GetInstance()->GetCPUCoreNumInformation(
          cpuCoreNumInfo)) {
    LOG_WARNING(sLogger,
                ("failed to get cpu core num", "invalid System collector"));
    return false;
  }
  ...
  systemLoadInfo.systemStat.load1PerCore =
      systemLoadInfo.systemStat.load1 /
      static_cast<double>(cpuCoreNumInfo.cpuCoreNum);
  systemLoadInfo.systemStat.load5PerCore =
      systemLoadInfo.systemStat.load5 /
      static_cast<double>(cpuCoreNumInfo.cpuCoreNum);
  systemLoadInfo.systemStat.load15PerCore =
      systemLoadInfo.systemStat.load15 /
      static_cast<double>(cpuCoreNumInfo.cpuCoreNum);

  return true;
}
bool LinuxSystemInterface::GetCPUCoreNumInformationOnce(
    CpuCoreNumInformation &cpuCoreNumInfo) {
  cpuCoreNumInfo.cpuCoreNum = std::thread::hardware_concurrency();
  cpuCoreNumInfo.cpuCoreNum =
      cpuCoreNumInfo.cpuCoreNum < 1 ? 1 : cpuCoreNumInfo.cpuCoreNum;
  return true;
}

static inline double Diff(double a, double b) { return a - b > 0 ? a - b : 0; }
...
Inactive:        1131312 kB
 */
bool LinuxSystemInterface::GetHostMemInformationStatOnce(
  ...
  meminfo.memStat.actualUsed =
      Diff(meminfo.memStat.total, meminfo.memStat.available);
  ...
  meminfo.memStat.usedPercent =
      meminfo.memStat.total > 0 ? diff * 100 / meminfo.memStat.total : 0.0;
...
// 已知文件系统
static const auto &knownFileSystem =
    *new std::unordered_map<std::string, FileSystemType>{
...
constexpr size_t fsTypeNamesCount =
    sizeof(fsTypeNames) / sizeof(fsTypeNames[0]);
static_assert(FILE_SYSTEM_TYPE_MAX == fsTypeNamesCount,
              "fsTypeNames size not matched");
...
bool GetFileSystemType(const std::string &fsTypeName, FileSystemType &fsType,
...
bool LinuxSystemInterface::GetFileSystemListInformationOnce(
  ...
  while (getmntent_r(fp, &ent, &buffer[0], buffer.size())) {
    ...
    GetFileSystemType(fileSystem.sysTypeName, fileSystem.type,
                      fileSystem.typeName);
...
第二列: 系统空闲的时间（以秒为单位）。
*/
bool LinuxSystemInterface::GetSystemUptimeInformationOnce(
  ...
  int ret = GetFileLines(PROCESS_DIR / PROCESS_UPTIME, uptimeLines, true,
                         &errorMessage);
...
bool LinuxSystemInterface::GetDiskSerialIdInformationOnce(
    std::string diskName, SerialIdInformation &serialIdInfo) {
  ...
  if (!CheckExistance(SYSTEM_BLOCK_DIR / diskName / SERIAL)) {
    LOG_ERROR(sLogger, ("file does not exist",
                        (SYSTEM_BLOCK_DIR / diskName / SERIAL).string()));
    return false;
  }
...
template <typename T> T DiffOrZero(const T &a, const T &b) {
  return a > b ? a - b : T{0};
}
bool LinuxSystemInterface::GetDiskStateInformationOnce(
  ...
  if (!CheckExistance(PROCESS_DIR / PROCESS_DISKSTATS)) {
    LOG_ERROR(sLogger, ("file does not exist",
                        (PROCESS_DIR / PROCESS_DISKSTATS).string()));
    return false;
  }
  int ret = GetFileLines(PROCESS_DIR / PROCESS_DISKSTATS, diskLines, true,
                         &errorMessage);
  if (ret != 0 || diskLines.empty()) {
    ...
    for (auto const &diskLine : diskLines) {
      ...
      try {
        ...
        auto end = parser.end();
        for (size_t i = 0;
             i < static_cast<size_t>(EnumDiskState::count) && iter != end;
             ++i, ++iter) {
          uint64_t value;
        ...
        // 直接从数组索引访问，零遍历开销
        diskStat.major = static_cast<unsigned int>(
            diskValues[static_cast<size_t>(EnumDiskState::major)]);
        diskStat.minor = static_cast<unsigned int>(
            diskValues[static_cast<size_t>(EnumDiskState::minor)]);

        // 4  reads completed successfully
        diskStat.reads = diskValues[static_cast<size_t>(EnumDiskState::reads)];
        // 6  sectors read
        diskStat.readBytes =
            diskValues[static_cast<size_t>(EnumDiskState::readSectors)] * 512;
        // 7  time spent reading (ms)
        diskStat.rTime =
            diskValues[static_cast<size_t>(EnumDiskState::rMillis)];
        // 8  writes completed
        diskStat.writes =
            diskValues[static_cast<size_t>(EnumDiskState::writes)];
        // 10  sectors written
        diskStat.writeBytes =
            diskValues[static_cast<size_t>(EnumDiskState::writeSectors)] * 512;
        // 11  time spent writing (ms)
        diskStat.wTime =
            diskValues[static_cast<size_t>(EnumDiskState::wMillis)];
        // 13  time spent doing I/Os (ms)
        diskStat.time =
            diskValues[static_cast<size_t>(EnumDiskState::rwMillis)];
        // 14  weighted time spent doing I/Os (ms)
        diskStat.qTime =
            diskValues[static_cast<size_t>(EnumDiskState::qMillis)];
...
bool LinuxSystemInterface::GetFileSystemInformationOnce(
    std::string dirName, FileSystemInformation &fileSystemInfo) {
  struct statvfs buffer {};
  int status = statvfs(dirName.c_str(), &buffer);
  if (status != 0) {
    LOG_ERROR(sLogger, ("get filesystem infomation error",
                        dirName)("error status", status));
    return false;
  }
  ...
  fileSystemInfo.fileSystemState.avail =
      ((buffer.f_bavail * bsize) >> 1); // 非超级用户最大可使用的磁盘量
  fileSystemInfo.fileSystemState.used =
      DiffOrZero(fileSystemInfo.fileSystemState.total,
                 fileSystemInfo.fileSystemState.free);
  ...
  uint64_t total = fileSystemInfo.fileSystemState.used +
                   fileSystemInfo.fileSystemState.avail;
...
bool LinuxSystemInterface::GetTCPStatInformationOnce(
  ...
  if (ret) {
    ...
    tcpStatInfo.stat.tcpNonEstablished =
        (netState.tcpStates[TCP_NON_ESTABLISHED]);
  }
...
bool LinuxSystemInterface::GetNetInterfaceInformationOnce(
...
bool LinuxSystemInterface::GetProcessCmdlineStringOnce(
    pid_t pid, ProcessCmdlineString &cmdline) {
  ...
  if (!file.is_open()) {
    LOG_ERROR(sLogger,
              ("open process cmdline file", "fail")("file", processCMDline));
    return false;
  }
...
bool LinuxSystemInterface::GetProcessStatmOnce(
    pid_t pid, ProcessMemoryInformation &processMemory) {
  ...
  if (!file.is_open()) {
    LOG_ERROR(sLogger,
              ("open process statm file", "fail")("file", processStatm));
    return false;
  }
...
bool LinuxSystemInterface::GetProcessCredNameOnce(
    pid_t pid, ProcessCredName &processCredName) {
  ...
  if (!file.is_open()) {
    LOG_ERROR(sLogger,
              ("open process status file", "fail")("file", processStatus));
    return false;
  }
  ...
  while (std::getline(file, line) && !(getUID && getGID && getName)) {
    ...
    if (firstField == "Name:") {
      ...
      if (!parser.GetFieldAs<uint64_t>(1, uid) ||
          !parser.GetFieldAs<uint64_t>(2, euid)) {
      ...
      if (!parser.GetFieldAs<uint64_t>(1, gid) ||
          !parser.GetFieldAs<uint64_t>(2, egid)) {
  ...
  if (getgrgid_r(cred.gid, &grpbuffer, groupBuffer, sizeof(groupBuffer),
                 &grp) != 0) {
    return true;
  }
...
bool LinuxSystemInterface::GetExecutablePathOnce(
    pid_t pid, ProcessExecutePath &executePath) {
  std::filesystem::path procExePath =
      PROCESS_DIR / std::to_string(pid) / PROCESS_EXE;
  char buffer[4096];
...
bool LinuxSystemInterface::GetProcessOpenFilesOnce(pid_t pid,
                                                   ProcessFd &processFd) {
  std::filesystem::path procFdPath =
      PROCESS_DIR / std::to_string(pid) / PROCESS_FD;
  ...
  try {
    auto it = std::filesystem::directory_iterator(
        procFdPath, std::filesystem::directory_options::skip_permission_denied,
        ec);
    while (it != std::filesystem::directory_iterator()) {
      if (ec) {
        if (ec == std::errc::permission_denied) {
          LOG_WARNING(sLogger,
                      ("skipping fd due to permissions", procFdPath.string()));
        } else if (ec == std::errc::no_such_file_or_directory) {
          LOG_INFO(sLogger, ("process exited during fd collection", pid));
          break; // 进程退出，停止遍历
        } else {
          LOG_WARNING(sLogger, ("temporary fd error",
                                ec.message())("path", procFdPath.string()));
        }
        ec.clear();
        it.increment(ec);
        continue;
      ...
      try {
        ...
        LOG_WARNING(sLogger, ("failed to process fd entry",
                              it->path().string())("error", e.what()));
      }
...
bool LinuxSystemInterface::InitGPUCollectorOnce(const FieldMap &fieldMap) {
  ...
  if (!mDcgmCollector.IsLibraryLoaded()) {
    LOG_ERROR(sLogger, ("GPU collector initialization failed",
                        "DCGM library not loaded"));
    return false;
  }

  if (!mDcgmCollector.CanInitialize()) {
    if (mDcgmCollector.IsFullyInitialized()) {
      LOG_INFO(sLogger, ("GPU collector already initialized",
                         "skipping initialization"));
      return true;
    } else {
      LOG_ERROR(sLogger, ("GPU collector initialization failed",
                          "DCGM collector in invalid state"));
      return false;
    }
  }
...
bool LinuxSystemInterface::GetCgroupStatInformationOnce(
    CgroupStatInformation &cgroupStatInfo) {
  ...
  if (ret != 0 || cgroupLines.empty()) {
    LOG_ERROR(sLogger,
              ("failed to read cgroup file", "fail")("error", errorMessage));
    return false;
  }
  ...
  for (const auto &line : cgroupLines) {
    ...
    if (fields.size() >= 3) {
      ...
      if (StringTo(fields[2], numCgroups)) {
        ...
        LOG_WARNING(sLogger, ("failed to parse num_cgroups",
                              fields[2])("subsys", subsysName));
      }
    }
  }
...
bool LinuxSystemInterface::GetGPUInformationOnce(GPUInformation &gpuInfo) {
  if (!mDcgmCollector.IsFullyInitialized()) {
    LOG_ERROR(sLogger,
              ("GPU data retrieval failed", "DCGM collector not ready"));
    return false;
  }
  ...
  if (!success) {
    LOG_ERROR(sLogger,
              ("GPU data retrieval failed", "collection operation failed"));
  } else {
    LOG_DEBUG(sLogger, ("GPU data retrieval successful", "metrics collected")(
                           "gpu_count", gpuInfo.stats.size()));
  }

  return success;
}

bool LinuxSystemInterface::GetDentryStatInformationOnce(
    DentryStatInformation &dentryStatInfo) {
  ...
  if (ret != 0 || dentryLines.empty()) {
    LOG_ERROR(sLogger, ("failed to read dentry-state file",
  ...
  for (const auto &line : dentryLines) {
    ...
    if (fields.size() >= 5) {
      ...
      if (StringTo(fields[0], nrDentry) && StringTo(fields[1], nrUnused) &&
          StringTo(fields[2], ageLimit) && StringTo(fields[3], wantPages) &&
          StringTo(fields[4], nrNegative)) {
