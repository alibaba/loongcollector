...
DEFINE_FLAG_INT32(host_monitor_thread_pool_size,
                  "host monitor thread pool size", 3);
DEFINE_FLAG_INT32(host_monitor_max_blocked_count,
                  "host monitor max blocked count to restart", 5);
...
void HostMonitorInputRunner::UpdateCollector(
    const std::string &configName,
    const std::vector<CollectorInfo> &newCollectorInfos,
    QueueKey processQueueKey, size_t inputIndex) {
  for (size_t i = 0; i < newCollectorInfos.size(); ++i) {
    const auto &collectorName = newCollectorInfos[i].name;

    if (mCollectorCreatorMap.find(collectorName) ==
        mCollectorCreatorMap.end()) {
      LOG_ERROR(sLogger, ("host monitor", "collector not supported")(
                             "config", configName)("collector", collectorName));
      continue;
    }
    auto collector = mCollectorCreatorMap.at(collectorName)();

    auto collectContext = std::make_shared<HostMonitorContext>(
        configName, collectorName, processQueueKey, inputIndex,
    ...
    if (!collectContext->mCollector.Init(*collectContext)) {
      LOG_WARNING(sLogger, ("host monitor", "init collector failed")(
                               "collector", collectorName));
      continue;
    }
    if (collectContext->mCollectInterval.count() == 0 ||
        collectContext->mReportInterval.count() == 0) {
      LOG_ERROR(sLogger,
                ("host monitor",
                 "collect interval or report interval is 0, will not collect")(
                    "config", configName)("collector", collectorName));
      continue;
    }
    ...
    LOG_INFO(sLogger,
             ("host monitor", "add new collector")("collector", collectorName));
  }

  if (newCollectorInfos.size() > 0) {
    mRunningPipelineCount++;
    LoongCollectorMonitor::GetInstance()->SetAgentHostMonitorTotal(
        mRunningPipelineCount);
  }
}
...
void HostMonitorInputRunner::Init() {
  ...
  if (IsStopping()) {
    LOG_ERROR(sLogger,
              ("Init", "previous stop is not completed, return directly"));
    return;
  }
...
void HostMonitorInputRunner::Stop() {
  ...
  if (IsStopping()) {
    LOG_WARNING(
        sLogger,
        ("Stop", "previous stop operation still running, return directly"));
    return;
  }
  ...
  if (status == std::future_status::ready) {
    LOG_INFO(sLogger,
             ("HostMonitorInputRunner", "stop completed successfully"));
  } else {
    // Timeout - force process exit to prevent undefined behavior
    LOG_ERROR(sLogger, ("host monitor runner stop timeout 3 seconds",
                        "force exit process to ensure thread safety"));
  ...
  }
  {
    ...
    for (const auto &[key, runInfo] : mRegisteredCollector) {
      if (std::chrono::duration_cast<std::chrono::seconds>(
              now - runInfo.lastRunTime) >
          runInfo.interval * INT32_FLAG(host_monitor_max_blocked_count)) {
        LOG_WARNING(
            sLogger,
            ("host monitor", "collector blocked")("collector",
                                                  key.collectorName)(
...
bool HostMonitorInputRunner::IsCollectTaskValid(
    const std::chrono::steady_clock::time_point &startTime,
    const std::string &configName, const std::string &collectorName) {
  std::shared_lock<std::shared_mutex> lock(mRegisteredCollectorMutex);
  CollectorKey key{configName, collectorName};
...
void HostMonitorInputRunner::ScheduleOnce(CollectContextPtr context) {
  auto collectFn = [this, context,
    ...
    try {
      ...
      if (context->ShouldGenerateMetric()) {
        ...
        if (result) {
          LOG_DEBUG(sLogger, ("host monitor", "collect data")(
                                 "collector", context->mCollectorName)(
          ...
          LOG_ERROR(sLogger,
                    ("host monitor collect data failed",
                     "collect error")("collector", context->mCollectorName));
          ADD_COUNTER(mDropItemsTotal, group.GetEvents().size());
          CollectorMetrics::GetInstance()->UpdateFailMetrics(
      ...
      LOG_ERROR(sLogger,
                ("host monitor collect data failed", "collect error")(
                    "collector", context->mCollectorName)("error", e.what()));
      CollectorMetrics::GetInstance()->UpdateFailMetrics(
          context->mCollectorName);
    }
                    ...
                    std::chrono::steady_clock::now() - startTime));
    {
      ...
      auto it = mRegisteredCollector.find(key);
      if (it == mRegisteredCollector.end() ||
          it->second.startTime != context->mStartTime) {
        LOG_DEBUG(sLogger, ("old collector is removed, will not collect again",
                            "discard data")("config", context->mConfigName)(
                               "collector", context->mCollectorName));
        return;
      }
...
void HostMonitorInputRunner::PushQueue(CollectContextPtr context,
  ...
  bool pushResult = ProcessorRunner::GetInstance()->PushQueue(
      context->mProcessQueueKey, context->mInputIndex, std::move(group));
  if (!pushResult) {
    LOG_ERROR(sLogger, ("host monitor push process queue failed",
...
void HostMonitorInputRunner::PushNextTimerEvent(CollectContextPtr context) {
  if (context->CheckClockRolling()) {
    ...
    std::chrono::steady_clock::time_point nextScheduleTime =
        context->GetScheduleTime() + context->mCollectInterval;
    time_t nextMetricTime =
        context->GetMetricTime() + context->mCollectInterval.count();
    int64_t skipCount = 0;
    if (now > nextScheduleTime) {
      ...
      LOG_WARNING(sLogger,
                  ("host monitor skip collect", "may casue data unaccurate")(
                      "collector", context->mCollectorName)("skip count",
                                                            skipCount + 1));
      if (context->mCollectType == HostMonitorCollectType::kMultiValue) {
        context->mCount =
            (context->mCount + skipCount + 1) % context->mCountPerReport;
      }
    }
...
#else
  auto hostIP =
      group.GetSourceBuffer()->CopyString(LoongCollectorMonitor::mIpAddr);
  for (auto &e : group.MutableEvents()) {
    if (!e.Is<MetricEvent>()) {
      continue;
    }
    auto &metricEvent = e.Cast<MetricEvent>();
    metricEvent.SetTagNoCopy(DEFAULT_HOST_IP_LABEL,
                             StringView(hostIP.data, hostIP.size));
  }
#endif
}
...
void HostMonitorInputRunner::InitMetrics() {
  ...
  mOutItemsTotal = mMetricsRecordRef.CreateCounter(
      METRIC_RUNNER_HOST_MONITOR_OUT_ITEMS_TOTAL);
  mOutItemsSize = mMetricsRecordRef.CreateCounter(
      METRIC_RUNNER_HOST_MONITOR_OUT_ITEMS_SIZE);
  mDropItemsTotal = mMetricsRecordRef.CreateCounter(
      METRIC_RUNNER_HOST_MONITOR_DROP_ITEMS_TOTAL);
  mLatencyTimeMs = mMetricsRecordRef.CreateTimeCounter(
      METRIC_RUNNER_HOST_MONITOR_LATENCY_TIME_MS);
