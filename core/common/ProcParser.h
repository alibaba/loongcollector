...
public:
  ...
  std::vector<uint32_t> GetUids() const {
    return ConvertToInt(uids);
  }

  std::vector<uint32_t> GetGids() const {
    return ConvertToInt(gids);
  }

  uint32_t GetLoginUid() const {
    return ConvertToInt(login_uid);
  }
