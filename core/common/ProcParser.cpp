...
std::tuple<std::string, int> ProcParser::LookupContainerId(const std::string& cgroup, bool bpfSource, bool walkParent) const {
  ...
  for (int j = subDirs.size() - 2; j > 1; --j) {
    ...
    if (container.size() == ContainerIdLength || (container.size() > ContainerIdLength && container.find("scope") != std::string::npos)) {
      return {container.substr(0, BpfContainerIdLength), i};
    }
  }
  return {"", 0};
...
std::tuple<std::string, int> ProcParser::ProcsFindDockerId(const std::string& cgroups) const {
  ...
  for (const auto& s : cgrpPaths) {
    if (s.find("pods") != std::string::npos || s.find("docker") != std::string::npos || s.find("libpod") != std::string::npos) {
