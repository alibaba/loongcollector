...
//     networkSecurityStat.plugin_type_ = logtail::ebpf::PluginType::NETWORK_SECURITY;
//     nami::eBPFStatistics processSecurityStat = ebpfStat;
//     processSecurityStat.plugin_type_ = logtail::ebpf::PluginType::PROCESS_SECURITY;
//     nami::eBPFStatistics fileSecurityStat = ebpfStat;
//     fileSecurityStat.plugin_type_ = logtail::ebpf::PluginType::FILE_SECURITY;

//     nami::NetworkObserverStatistics networkStat;
//     networkStat.updated_ = true;
//     networkStat.plugin_type_ = logtail::ebpf::PluginType::NETWORK_OBSERVE;
//     networkStat.updated_ = true;
...
//     EXPECT_EQ(ebpf::eBPFServer::GetInstance()->mMonitorMgr->mInited[int(logtail::ebpf::PluginType::NETWORK_OBSERVE)], true);
//     auto& mgr = ebpf::eBPFServer::GetInstance()->mMonitorMgr->mSelfMonitors[int(logtail::ebpf::PluginType::NETWORK_OBSERVE)];
//     auto mgrPtr = mgr.get();
...
//     auto network_conf = std::get<logtail::ebpf::NetworkObserveConfig>(conf->config_);
//     EXPECT_EQ(conf->mPluginType, logtail::ebpf::PluginType::NETWORK_OBSERVE);
...
//     EXPECT_TRUE(ebpf::eBPFServer::GetInstance()->mSourceManager->mRunning[int(logtail::ebpf::PluginType::NETWORK_OBSERVE)]);

//     // do suspend
//     ebpf::eBPFServer::GetInstance()->SuspendPlugin("test", logtail::ebpf::PluginType::NETWORK_OBSERVE);
...
//     input->SetMetricsRecordRef("test", "2");
//     res = ebpf::eBPFServer::GetInstance()->EnablePlugin(
//         "test", 8, logtail::ebpf::PluginType::NETWORK_OBSERVE, &ctx, &network_option, input->mPluginMgr);
//     EXPECT_TRUE(res);
...
//     auto after_conf = std::get<logtail::ebpf::NetworkObserveConfig>(conf->config_);
//     EXPECT_EQ(ebpf::eBPFServer::GetInstance()->mMeterCB->mQueueKey, ctx.GetProcessQueueKey());
//     EXPECT_EQ(ebpf::eBPFServer::GetInstance()->mSpanCB->mQueueKey, ctx.GetProcessQueueKey());
...
//     // do stop
//     ebpf::eBPFServer::GetInstance()->DisablePlugin("test", logtail::ebpf::PluginType::NETWORK_OBSERVE);
//     EXPECT_EQ(ebpf::eBPFServer::GetInstance()->mSpanCB->mQueueKey, -1);
//     EXPECT_TRUE(!ebpf::eBPFServer::GetInstance()->mSourceManager->mRunning[int(logtail::ebpf::PluginType::NETWORK_OBSERVE)]);
// }
...
//         "test", 0, logtail::ebpf::PluginType::PROCESS_SECURITY, &ctx, &security_options, input->mPluginMgr);
//     EXPECT_TRUE(res);
//     auto conf = ebpf::eBPFServer::GetInstance()->mSourceManager->mConfig.get();
//     EXPECT_EQ(conf->mPluginType, logtail::ebpf::PluginType::PROCESS_SECURITY);
//     EXPECT_EQ(conf->type, UpdataType::SECURE_UPDATE_TYPE_ENABLE_PROBE);
//     auto process_conf = std::get<logtail::ebpf::ProcessConfig>(conf->config_);
...
//     // do suspend
//     ebpf::eBPFServer::GetInstance()->SuspendPlugin("test", logtail::ebpf::PluginType::PROCESS_SECURITY);
...
//     EXPECT_TRUE(ebpf::eBPFServer::GetInstance()->mSourceManager->mRunning[int(logtail::ebpf::PluginType::PROCESS_SECURITY)]);
...
//     EXPECT_EQ(conf->mPluginType, logtail::ebpf::PluginType::PROCESS_SECURITY);
//     EXPECT_EQ(conf->type, UpdataType::SECURE_UPDATE_TYPE_CONFIG_CHAGE);
//     auto after_conf = std::get<logtail::ebpf::ProcessConfig>(conf->config_);
...
//     EXPECT_TRUE(res);
//     auto conf = ebpf::eBPFServer::GetInstance()->mSourceManager->mConfig.get();
//     EXPECT_EQ(conf->mPluginType, logtail::ebpf::PluginType::NETWORK_SECURITY);
//     EXPECT_EQ(conf->type, UpdataType::SECURE_UPDATE_TYPE_ENABLE_PROBE);
//     auto inner_conf = std::get<logtail::ebpf::NetworkSecurityConfig>(conf->config_);
...
//     auto filter = std::get<logtail::ebpf::SecurityNetworkFilter>(inner_conf.options_[0].filter_);
...
//     // do suspend
//     ebpf::eBPFServer::GetInstance()->SuspendPlugin("test", logtail::ebpf::PluginType::NETWORK_SECURITY);
...
//     EXPECT_TRUE(ebpf::eBPFServer::GetInstance()->mSourceManager->mRunning[int(logtail::ebpf::PluginType::NETWORK_SECURITY)]);

//     input->SetContext(ctx);
//     input->SetMetricsRecordRef("test", "2");
//     res = ebpf::eBPFServer::GetInstance()->EnablePlugin(
//         "input_network_security", 0, logtail::ebpf::PluginType::NETWORK_SECURITY, &ctx, &security_options, input->mPluginMgr);
//     EXPECT_TRUE(res);
//     conf = ebpf::eBPFServer::GetInstance()->mSourceManager->mConfig.get();
//     EXPECT_EQ(conf->mPluginType, logtail::ebpf::PluginType::NETWORK_SECURITY);
//     EXPECT_EQ(conf->type, UpdataType::SECURE_UPDATE_TYPE_CONFIG_CHAGE);

//     auto after_conf = std::get<logtail::ebpf::NetworkSecurityConfig>(conf->config_);
...
//     EXPECT_TRUE(res);
//     auto conf = ebpf::eBPFServer::GetInstance()->mSourceManager->mConfig.get();
//     EXPECT_EQ(conf->mPluginType, logtail::ebpf::PluginType::FILE_SECURITY);
//     EXPECT_EQ(conf->type, UpdataType::SECURE_UPDATE_TYPE_ENABLE_PROBE);
//     auto inner_conf = std::get<logtail::ebpf::FileSecurityConfig>(conf->config_);
//     EXPECT_TRUE(inner_conf.file_security_cb_ != nullptr);
//     EXPECT_EQ(inner_conf.options_.size(), 1);
//     EXPECT_EQ(inner_conf.options_[0].call_names_.size(), 3);
//     auto filter = std::get<logtail::ebpf::SecurityFileFilter>(inner_conf.options_[0].filter_);
...
//     // do suspend
//     ebpf::eBPFServer::GetInstance()->SuspendPlugin("test", logtail::ebpf::PluginType::FILE_SECURITY);
...
//     EXPECT_TRUE(ebpf::eBPFServer::GetInstance()->mSourceManager->mRunning[int(logtail::ebpf::PluginType::FILE_SECURITY)]);
...
//     auto after_conf = std::get<logtail::ebpf::FileSecurityConfig>(conf->config_);
