...
|  FilterKey  |  \[string\]  |  否  |  /  |  需与`FilterRegex`配套，等长，含义为字段必须匹配对应正则；与 `FilterRegex` 同时提供。  |
|  FilterRegex  |  \[string\]  |  否  |  /  |  与 `FilterKey` 对应的正则，必须与其长度相同。  |
|  Include  |  map[string]string  |  否  |  /  |  键为字段名，值为正则表达式。  |
