...
> v2 场景下对每条 Log 事件进行字段值映射（输入 Log 输出 Log），Metric/Span 事件显式透传（不处理但不丢弃）。
...
配置 **LoongCollector** 时，请将映射表文件（如上文的 `ipExample.csv`）放在用户自有目录下（例如 `/home/ipExample.csv`），而不要放在 **Agent 安装/配置目录**中：部分历史布局下该目录名仍为 `ilogtail`，升级或重装时可能被清空。阿里云帮助中心仍以「Logtail」描述控制台侧能力，可参考[使用 Logtail 插件处理数据](https://help.aliyun.com/document_detail/64957.html)中的路径约定。
...

{
    "_ip_": "127.0.0.1",
    "Index": "100000004",
    "_processed_ip_": "LocalHost-LocalHost",
    "__time__": "1627004588"
}
