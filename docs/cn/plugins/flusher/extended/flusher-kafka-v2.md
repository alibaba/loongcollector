...
| Authentication.TLS.Enabled            | bool  | 否    | 是否启用TLS安全连接,                                                                                               |
...
| Authentication.TLS.InsecureSkipVerify | bool  | 否    | 是否跳过TLS证书校验                                                                                                |
| Authentication.Kerberos.ServiceName   | string   | 否    | 服务名称，例如：kafka                                                                                              |
| Authentication.Kerberos.UseKeyTab     | bool  | 否    | 是否采用keytab，配置此项后需要配置KeyTabPath，默认为：`false`                                                                 |
| Authentication.Kerberos.Username      | bool  | 否    | UseKeyTab设置为`false`的情况下，需要指定用户名                                                                            |
...
| Authentication.Kerberos.ConfigPath    | bool  | 否    | Kerberos krb5.conf                                                                                         |
