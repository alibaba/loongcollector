...
| `processor_default`<br>[原始数据](processor/extended/processor-default.md) | SLS 官方 | ✓ | ✓ | dual | 透传未知 | 透传未知 | 不对数据任何操作，只是简单的数据透传。 |
...
| `processor_json`<br>[Json](processor/extended/processor-json.md) | SLS 官方 | ✓ | ✓ | dual | 透传未知 | 透传未知 | 实现对 Json 格式日志的解析。 |
...
| `processor_rename`<br>[重命名字段](processor/extended/processor-rename.md) | SLS 官方 | ✓ | ✓ | dual | 透传未知 | 透传未知 | 重命名字段。 |
...
| `flusher_http`<br>[HTTP](flusher/extended/flusher-http.md) | 社区<br>[snakorse](https://github.com/snakorse) | ✓ | ✓ | dual | 透传未知 | 透传未知 | 将采集到的数据以 http 方式输出到指定的后端。 |
