...
3. 最终的 YAML 如下。将其保存到运行目录下的 `conf/continuous_pipeline_config/local`，LoongCollector 会自动加载该**采集配置**。

> **提示**：如需将自监控指标发送到 Go Pipeline（例如 `flusher_prometheus`），请参见[如何将自监控数据发送到Go Pipeline](../how-to-export-self-monitor-data.md)。
