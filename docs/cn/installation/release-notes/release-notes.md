...
- **性能与数据路径**：集成 **simdjson**；string_view 解析、span 序列化；按字节限界的 process queue；减少 group 创建；发送侧并发与退火、队列重试与指标增强。
...
4. [全新的管控协议](https://github.com/alibaba/loongcollector/blob/main/config_server/protocol/v2/README.md)
    ...
    * 管控范围：从仅持续采集配置（collection pipeline）扩展为**持续采集配置**、**一次性采集配置**与**实例级配置（instance_config）**的统一管控。
