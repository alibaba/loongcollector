...
func HandleLoadConfig(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		logger.Error(context.Background(), util.CategoryConfigAlarm, "stage", "read", "err", err)
		w.WriteHeader(500)
	...
	if err != nil {
		logger.Error(context.Background(), util.CategoryConfigAlarm, "stage", "parse", "err", err)
		w.WriteHeader(500)
	...
	once.Do(func() {
		...
		if len(handlers) != 0 {
			...
			go func() {
				...
				logger.Info(context.Background(), "#####################################")
				logger.Error(context.Background(), util.InitHttpServerAlarm, "init http server err", http.ListenAndServe(*flags.HTTPAddr, mux)) //nolint
			}()
		}
	})
}
