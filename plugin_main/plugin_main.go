...
func changePluginConfigIO(pluginCfg string) string {
	if *flags.FileIOFlag {
		...
		if err := json.Unmarshal([]byte(pluginCfg), &newCfg); err == nil {
			...
			logger.Error(context.Background(), util.CATEGORY_CONFIG_ALARM, "err", err)
		}
		return pluginCfg
	}
	return pluginCfg
...
func main() {
	...
	for i, cfg := range pluginCfgs {
		...
		if LoadPipeline(p, l, c, 123, cfg) != 0 {
			logger.Warningf(context.Background(), util.CATEGORY_CONFIG_ALARM, "%s_%s_%s start fail, config is %s", p, l, c, cfg)
			return
		}
		Start(c)
	}
